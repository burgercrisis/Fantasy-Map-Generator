...
- Decision (for now): keep `culture.race` as a legacy/cached field (avoid breaking older maps / remaining call sites), but do not treat it as authoritative when `cells.race` is present.
 ...
 - 2025-12-16: Fixed `modules/races.js` race-mixer refresh: `ensureRaceMixerBaseIndex` was referencing an undefined `seed` variable, which could throw and prevent refreshing race mixer base seed blobs / chains in some paths. Updated the function to derive a local `seed` from `options.seed`. Verified: `node --check modules/races.js` (exit 0).
 - 2025-12-17: UI/runtime verification (local `py -m http.server 3000`): `ensureRaceMixerBaseIndex('Elf', {seed: 'ui-verify'})` executes without `seed is not defined` errors and creates/updates a race mixer base with `raceMixerFor='Elf'` and a non-trivial seed blob (`blobCount≈200`).
 - 2025-12-17: Reduced console spam during generation: `modules/names-mixer.js` now logs `Mixed name is too short! Random name will be selected` at most once per session (previously could spam dozens of times) while keeping behavior unchanged (still falls back to a random seed from `baseConfig.b`). Verified: `node --check modules/names-mixer.js` (exit 0).
 - 2025-12-17: Reduced console spam during generation: `modules/names-generator.js` now logs `Namebase <n> is not found. First available namebase will be used` at most once per missing base index per session (behavior unchanged: still falls back to base `0`). Verified: `node --check modules/names-generator.js` (exit 0).
 - 2025-12-17: Fixed root cause of random missing-namebase warnings: `nameBases` is sparse, so `rand(nameBases.length...)` could pick holes (e.g. `5124`). Added `Names.getRandomBaseIndex()` in `modules/names-generator.js` and updated random-name call sites to use it (burg/river/lake/battle name randomizers, culture random set selection, era randomization, state/province editor random short names, and v1.3 auto-update). Verified: `node --check` on all touched files.
 - 2025-12-17: Fixed race mixer bases not being applied (classic/unmixed output): `modules/races.js` now replaces a culture’s `base` with the generated race-mixer base during `initializeRacesForExpansion` and `syncCultureBasesToDominantRace` when the culture is still using a classic fantasy race base. Verified: `node tools/mixer-races/check-race-language-profiles.js` (exit 0).
 - 2025-12-17: Added `Scions` as a race and wired it into the race language mixer (`modules/races.js`), including a unique fallback fantasy base (`i:274`) and an expanded race language profile (Uralic/Sami, Slavic/Baltic, Turkic/Mongolic, Native American families, Koreanic, isolates).
