...
- ✅ **2025-12-18 Decluster bases=[1,13] (Dutch creoles: negerhollands / negro-dutch / skepi-dutch-creole):** applied `tools/mixer-deltas/2025-12-18-decluster-1-13-dutch-creoles.json` to set `negerhollands` bases to `[1,13,0]`, `negro-dutch` to `[1,13,6]`, and keep `skepi-dutch-creole` at `[1,13]`; regenerated artifacts via `pnpm run mixer:apply-deltas`; verified via `pnpm exec -- node tools/mixer-diagnostics/report-language-mixer-base-clusters.js --min-size=3 --include-families` (no `bases=[1,13]` size>=3 cluster) and `pnpm exec -- node tools/check-language-mixer-map-inconsistencies.js --show-all-bases` (exit 0).
