
 {
 "i": 18,
 "max": 15,
 "m": 0,
 },
 "name": "Ekoka !Kung",
 "min": 3,
 "d": "lnrtkxgms",
 "b": "Kgalagadi,Kumune,Epukiro,Gxai,Gobabis,Karibib,Tsabis,Nossob,Leonardsville,Tses,Aminuis,Aroab,Grootfontein,Wilhelmstal,Araub,Witvlei,Stampriet,Mariental,Kalahari,Tsumkwe,Blouputs,Gaborone,Francistown,Molepolole,Mogoditshane,Maun,Selebi-Phikwe,Kanye,Serowe,Mahalapye,Palapye,Tlokweng,Ramotswa,Lobatse,Ghanzi,Bobonong,Letlhakane,Moshupa,Thamaga,Jwaneng,Gabane,Kopong,Kasane,Tshabong,Shoshong,Kazungula,Good Hope,Maitengwe,Sefhare,Mmathethe,Mathangwane,Hukuntsi,Ramokgonami,Ramotswa station/Taung,Lecheng,Sebina,Morwa,Bray,Letlhakeng,Mochudi,Otse,Shakawe,Metsemotlhaba,Rakops,Tshane,Makalamabedi,Manyana,Dekar,Ghanzi District,Central District,Kgalagadi District"
 {
 "i": 40,
 "max": 9,
 "m": 0,
 },
 "name": "Nng Click",
 "min": 3,
 "d": "lnrtkxgms",
 "b": "Upington,Kuruman,Kakamas,Keimoes,Springbok,Pofadder,Kimberley,DeAar,Hartswater,Griquatown,BarklyWest,Douglas,Campbell,Prieska,VictoriaWest,Colesberg"
 {
 "i": 46,
 "max": 9,
 "m": 0,
 },
 "name": "Melanesian Vanuatu",
 "min": 4,
 "d": "nic-GH",
 "b": "Port Vila,Luganville,Norsup,Sola,Saratamata,Longana,Lakatoro,Norfolk,Isangel,Lenakel,Litzlitz,Loltong,Mota Lava,Port Olry,Whitesands,Avire,Lamap,Ipota"
 {
 "i": 54,
 "max": 12,
 "m": 0,
 },
 "name": "Central Pacific",
 "min": 4,
 "d": "nic-GH",
 "b": "Apia,Faleula,Vaitele,Siusega,Malie,Afega,Fasitoo,Uafato,Nuku'alofa,Neiafu,Pangai,Hakahau,Matavai,Papeete,Punaauia,Faaa,Paopao,Uturoa,Taiohae,TaiohaeVillage,Siumu,Safotu,Salelologa,Faleasiu,Fasitoo Uta,Leulumoega,Nofoali'i,Falefa,Falealili,Falealupo,Faleatiu"
 {
 "i": 70,
 "max": 12,
 "m": 0,
 },
 "name": "Purepecha",
 "min": 5,
 "d": "nic-GH",
 "b": "Morelia,Uruapan,Zamora,Patzcuaro,Tzintzuntzan,Cheran,Paracho,LosReyes,Angamacutiro,Angangueo,Apatzingan,Acuitzio,Aguililla,Arteaga,Ario,Brisenas,Buenavista,Caracuaro,Charapan,Charo,Chavinda,Chilchota,Chinicuila,Chucandiro,Churintzio,Churumuco,Coahuayana,Coalcoman,Coeneo,Cojumatlan,Contepec,Copandaro,Cotija,Cuitzeo,Ecuandureo,Erongaricuaro,GabrielZamora,Hidalgo,Huandacareo,Huaniqueo,Huetamo,Huiramba,Indaparapeo,Irimbo,Ixtlan,Jacona,Jimenez,Jiquilpan,Jungapeo,Lagunillas,LaPiedad,Maravatio,Morelos,Quiroga,Santa Fe de la Laguna,Ihuatzio,Cuanajo,Capula,Janitzio,Tingüindín,Nuevo Parangaricutiro,Senguio,Tacámbaro,Santa Clara del Cobre,Tancítaro,Peribán,Tepalcatepec"
 {
 "i": 73,
 "max": 11,
 "m": 0,
 },
 "name": "Forro Sao Tome",
 "min": 5,
 "d": "nic-GH",
 "b": "Sao Tome,Trindade,Santana,Neves,Guadalupe,Bombom,Pantufo,agua Grande,Riboque,Mesquita,Batepa,Piedade,Ribeira Afonso,Ribeira Peixe,Madalena,Micolo,Monte Cafe,Conde,Boa Entrada,Boa Vista,Santa Margarida,Santa Catarina,Santa Cruz,Santa Clara,Santa Luzia,Diogo Vaz,Porto Alegre,Praia Cruz,Palmares,agua Ize,Fruta Fruta,Sao Marcal,Sao Miguel,Sao Pedro,Uba Budo,Emolve,Monte Mario,Caixao Grande,Almas,Alto Douro,Sao Lazaro,Almeirim,Andrade,Bobo Forro,Bela Vista,Bom Sucesso,Bom Bom,Bom Despacho,São João dos Angolares,Angolares,Ananas,Monte Café"
 {
 "i": 78,
 "max": 11,
 "m": 0,
 },
 "name": "Huave",
 "min": 4,
 "d": "nic-GH",
 "b": "San Mateo del Mar,San Dionisio del Mar,San Francisco del Mar,Santa Maria del Mar,Tehuantepec,Santo Domingo Tehuantepec,Salina Cruz,Juchitan de Zaragoza,Juchitan,Union Hidalgo,San Blas Atempa,San Pedro Huamelula,San Pedro Tapanatepec,Chahuites,San Francisco Ixhuatan,Asuncion Ixtaltepec,Matias Romero,Tonala,Arriaga,El Espinal,Santiago Niltepec,Ciudad Ixtepec,Guevea de Humboldt,Reforma de Pineda,Santa Maria Chimalapa,San Miguel Chimalapa,Magdalena Tequisistlan,San Juan Guichicovi"
 {
 "i": 83,
 "max": 11,
 "m": 0,
 },
 "name": "Yanomami Amazonian",
 "min": 4,
 "d": "nic-GH",
 "b": "Boa Vista,Caracaraa,Mucajaa,Alto Alegre,Pacaraima,San Gabriel da Cachoeira,Barcelos,Santa Isabel do Rio Negro,Maroa,La Esmeralda,Santa Elena de Uairen,Maturaca,Auaris,Surucucu,Parima,Catrimani,Demini,Mucajai,Tepequem"
 {
 "i": 123,
 "max": 11,
 "m": 0,
 },
 "name": "Bima",
 "min": 4,
 "d": "nic-GH",
 "b": "Bima,Raba,Woha,Sape,Wera,Lambu,Wawo,Sanggar,Tambora,Dompu,Pekat,Soriutu,Kempo,Madapangga"
 {
 "i": 137,
 "max": 11,
 "m": 0,
 },
 "name": "Basap",
 "min": 4,
 "d": "nic-GH",
 "b": "Kutai Timur,Sangatta,Bengalon,Muara Bengalon,Rantau Pulung,Bontang,Samarinda,Tenggarong,Muara Wahau,Telen,Kaubun,Sandaran,Long Mesangat"
 {
 "i": 139,
 "max": 11,
 "m": 0,
 },
 "name": "Land Dayak",
 "min": 4,
 "d": "nic-GH",
 "b": "Kuching,Bau,Serian,Padawan,Penrissen,Siburan,Kota Samarahan,Tebedu,Lundu,Sematan,Serikin,Bongawan,Bidayuh,Mambong"
 {
 "i": 142,
 "max": 11,
 "m": 0,
 },
 "name": "Sumba-Flores",
 "min": 4,
 "d": "nic-GH",
 "b": "Waingapu,Waikabubak,Tambolaka,Kodi,West Sumba,East Sumba,Ruteng,Labuan Bajo,Komodo,Ende,Bajawa,Maumere,Ngada,Manggarai"
 {
 "i": 145,
 "max": 11,
 "m": 0,
 },
 "name": "Muna-Buton",
 "min": 4,
 "d": "nic-GH",
 "b": "Raha,Muna,Buton,Baubau,Pasarwajo,Lasalimu,Wakorumba,Wangi-Wangi,Kendari,Wolio,Kapontori,Batauga"
 {
 "i": 149,
 "max": 11,
 "m": 0,
 },
 "name": "Melanau-Kajang",
 "min": 4,
 "d": "nic-GH",
 "b": "Mukah,Dalat,Matu,Daro,Igan,Oya,Balingian,Selangau,Sibu,Bintulu,Tatau,Samalaju,Sarikei,Kanowit"
 {
 "i": 153,
 "max": 11,
 "m": 0,
 },
 "name": "Greater North Borneo",
 "min": 4,
 "d": "nic-GH",
 "b": "Borneo,Sabah,Sarawak,Brunei,Labuan,Kuching,Miri,Sibu,Bintulu,Kota Kinabalu,Sandakan,Tawau,Kudat,Kapit,Limbang,Beaufort,Sipitang,Keningau,Tenom,Ranau,Kota Belud,Tuaran,Papar,Putatan,Penampang,Lahad Datu,Semporna"
 {
 "i": 159,
 "max": 11,
 "m": 0,
 },
 "name": "Seko-Badaic",
 "min": 4,
 "d": "nic-GH",
 "b": "Seko,Limbong,Rampi,Rongkong,Seko Padang,Seko Tengah,Seko Baras,Masamba,Palopo,Malili,Rantepao,Enrekang,Gintu,Bada,Behoa,Napu,Watutau,Wuasa,Doda,Sangginora,Kaduwaa,Lengkeka,Tuare,Bomba"
 {
 "i": 163,
 "max": 11,
 "m": 0,
 },
 "name": "Northwest Sumatra Barrier Islands",
 "min": 4,
 "d": "nic-GH",
 "b": "Nias,Gunungsitoli,Teluk Dalam,Simeulue,Sinabang,Banyak Islands,Singkil,Sibolga,Mentawai,Tuapejat,Sikakap,Siberut,Pagai"
 {
 "i": 165,
 "max": 11,
 "m": 0,
 },
 "name": "SHWNG",
 "min": 4,
 "d": "nic-GH",
 "b": "Halmahera,Ternate,Tidore,Bacan,Obi,Weda,Patani,Gebe,Buli,Gane,Sorong,Manokwari,Raja Ampat,Waigeo,Misool,Salawati,Batanta,Kofiau,Gag,Sayang,Ayau,Fani,Mapia,Biak,Numfor,Yapen,Waropen"
 {
 "i": 167,
 "max": 11,
 "m": 0,
 },
 "name": "Bali Sasak Sumbawa",
 "min": 4,
 "d": "nic-GH",
 "b": "Denpasar,Ubud,Singaraja,Gianyar,Amlapura,Mataram,Senggigi,Praya,Selong,Lombok,Sumbawa Besar,Bima,Dompu"
 {
 "i": 179,
 "max": 11,
 "m": 0,
 },
 "name": "Tiwi",
 "min": 4,
 "d": "nic-GH",
 "b": "Wurrumiyanga,Nguiu,Pirlangimpi,Milikapiti,Snake Bay,Garden Point,Bathurst Island,Melville Island,Pickertaramoor,Ranku,Kulama,Putjamirra,Andranangoo,Apsley Strait,Fort Dundas"
 {
 "i": 195,
 "max": 11,
 "m": 0,
 },
 "name": "Angal",
 "min": 4,
 "d": "nic-GH",
 "b": "Mendi,Tari,Ialibu,Porgera,Wabag,Mount Hagen,Kundiawa,Goroka,Kagua,Nipa,Pangia,Kompiam,Koroba,Tambul,Erave"
 {
 "i": 200,
 "max": 11,
 "m": 0,
 },
 "name": "Bimin",
 "min": 4,
 "d": "nic-GH",
 "b": "Tabubil,Telefomin,Oksapmin,Kiunga,Ok Tedi,Fly River,Vanimo,Aitape,Wewak,Sandaun,Green River,Imonda,Bimin,Lumi,Nuku,Ambunti,Angoram,Maprik"
 {
 "i": 203,
 "max": 11,
 "m": 0,
 },
 "name": "Gahuku",
 "min": 4,
 "d": "nic-GH",
 "b": "Goroka,Kainantu,Gahuku,Asaro,Okapa,Watabung,Daulo,Marawaka,Lae,Madang,Wau,Bena Bena,Unggai,Lufa,Yonki,Sine,Kotuni,Henengaru,Alekano"
 {
 "i": 205,
 "max": 11,
 "m": 0,
 },
 "name": "Awiyaana",
 "min": 4,
 "d": "nic-GH",
 "b": "Daru,Kiunga,Western Province,Fly River,Balimo,Ok Tedi,Tabubil,Bamu River,Delta Fly,Lake Murray,Port Moresby,Wamena,Kurima,Jiwika,Wosilimo,Megapura,Pyramid,Bokondini,Karubaga,Kelila,Mamit,Awiyaana,Baliem"
 {
 "i": 207,
 "max": 11,
 "m": 0,
 },
 "name": "Kamoro",
 "min": 4,
 "d": "nic-GH",
 "b": "Timika,Mimika,Mapurujaya,Kuala Kencana,Atuka,Kaokonao,Kokonao,Nabire,Enarotali,Paniai,Carstensz,Pouweri,Tembagapura,Kamoro,Sempan,Yaraya"
 {
 "i": 209,
 "max": 11,
 "m": 0,
 },
 "name": "Kenati",
 "min": 4,
 "d": "nic-GH",
 "b": "Lae,Morobe,Bulolo,Wau,Watut,Markham Valley,Nadzab,Finschhafen,Madang,Goroka,Mount Hagen,Kenati,Hides,Nogoli,Koroba,Tari,Kopiago,Porgera,Kandep,Wabag,Laiagam,Wapenamanda,Kompiam"
 {
 "i": 214,
 "max": 11,
 "m": 0,
 },
 "name": "Babuza",
 "min": 4,
 "d": "nic-GH",
 "b": "Taichung,Changhua,Lukang,Yuanlin,Wufeng,Dadu,Shalu,Qingshui,Fengyuan,Taiping,Ershui,Tianzhong,Dacun,Xiushi"
 {
 "i": 216,
 "max": 11,
 "m": 0,
 },
 "name": "Castuo",
 "min": 4,
 "d": "nic-GH",
 "b": "Badajoz,Merida,Caceres,Plasencia,Don Benito,Villanueva de la Serena,Zafra,Almendralejo,Trujillo,Navalmoral de la Mata,Olivenza,Jerez de los Caballeros"
 {
 "i": 248,
 "max": 11,
 "m": 0,
 },
 "name": "Guernesiais",
 "min": 4,
 "d": "nic-GH",
 "b": "St Peter Port,St Sampson,Castel,St Martin,St Andrew,Forest,St Saviour,Torteval,Vale,Lancresse,Jerbourg,Pleinmont"
 {
 "i": 263,
 "max": 11,
 "m": 0,
 },
 "name": "Tsimane",
 "min": 4,
 "d": "nic-GH",
 "b": "San Borja,Yucumo,Rurrenabaque,Reyes,Santa Ana del Yacuma,Triple A,Beni,Yacuma,Bala,Yumano,Chimanes,Maniqui,Tsimane,San Ignacio,Santa Ana,Riberalta,Guayaramerin,Trinidad,Magdalena"
 {
 "i": 275,
 "max": 11,
 "m": 0,
 },
 "name": "Kallawaya",
 "min": 4,
 "d": "nic-GH",
 "b": "Charazani,Curva,Chajaya,Amarete,Kaata,Carabuco,Achacachi,Sorata,La Paz,Copacabana,Titicaca,Ancoraimes"
 {
 "i": 277,
 "max": 11,
 "m": 0,
 },
 "name": "Nivacle",
 "min": 4,
 "d": "nic-GH",
 "b": "Filadelfia,Loma Plata,Neuland,Mariscal Estigarribia,Teniente Irala Fernandez,Pozo Hondo,Fortin Boqueron,Villa Hayes,Chaco,Pilcomayo,Paraguay,Presidente Hayes"
 {
 "i": 363,
 "max": 11,
 "m": 0,
 },
 "name": "Anso",
 "min": 4,
 "d": "nic-GH",
 "b": "Angers,Saumur,Cholet,Nantes,Le Mans,Tours,Niort,Chinon"
 {
 "i": 385,
 "max": 11,
 "m": 0,
 },
 "name": "Chiac",
 "min": 4,
 "d": "nic-GH",
 "b": "Madrid,Toledo,ivila,Segovia,Valladolid,Zamora,Salamanca,Burgos,Soria"
 {
 "i": 396,
 "max": 11,
 "m": 0,
 },
 "name": "Chipilo",
 "min": 4,
 "d": "nic-GH",
 "b": "Chipilo,Puebla,Cholula,Atlixco,Huejotzingo,San Andres Cholula,San Pedro Cholula,Cuautlancingo,Tecali,Tepeaca,Amozoc,Tlaxcala"
 {
 "i": 435,
 "max": 11,
 "m": 0,
 },
 "name": "Franco-Provencal",
 "min": 4,
 "d": "ro-FR",
 "b": "Grenoble,Annecy,Chambery,Aix-les-Bains,Valence,Lyon,Vald'Aosta,Sion,Fribourg,Neuchatel,Geneva"
 {
 "i": 454,
 "max": 11,
 "m": 0,
 },
 "name": "Magoua",
 "min": 4,
 "d": "ro-NC",
 "b": "Noumea,Paita,Mont-Dore,Dumbea,Tchamba,Lifou,Thio,Bourail,Yate"
 {
 "i": 502,
 "max": 11,
 "m": 0,
 },
 "name": "Ch'orti'",
 "min": 4,
 "d": "may-GT",
 "b": "Chiquimula,Jocotan,Camotan,Olopa,Esquipulas,Ipala,Quezaltepeque,Zacapa,Rio Hondo,San Jacinto,La Union,San Juan Ermita"
 {
 "i": 522,
 "max": 11,
 "m": 0,
 },
 "name": "Peruvian Ribereno",
 "min": 4,
 "d": "es-PE",
 "b": "Iquitos,Nauta,Requena,Contamana,Yurimaguas,San Lorenzo,Caballococha,Tamshiyacu,Indiana,Mazan,Punchana,Belen,San Juan Bautista"
 {
 "i": 541,
 "max": 11,
 "m": 0,
 },
 "name": "Provencal",
 "min": 4,
 "d": "oc-FR",
 "b": "Marseille,Aix-en-Provence,Arles,Toulon,Avignon,Digne-les-Bains,Gap,Nice"
 {
 "i": 570,
 "max": 11,
 "m": 0,
 },
 "name": "Remois",
 "min": 4,
 "d": "ro-FR",
 "b": "Reims,Chalons-en-Champagne,Epernay,Vitry-le-Francois"
 {
 "i": 593,
 "max": 11,
 "m": 0,
 },
 "name": "Somontanes",
 "min": 4,
 "d": "es-ES",
 "b": "Barbastro,Monzon,Binefar,Graus,Tamarite de Litera,Alquezar,Abizanda,Adahuesca,Albalate de Cinca,Alberuela de Tubo,Alcampell,Alcolea de Cinca"
 {
 "i": 630,
 "max": 11,
 "m": 0,
 },
 "name": "Valdotain",
 "min": 4,
 "d": "nic-GH",
 "b": "Aosta,Saint-Vincent,Chatillon,Sarre,Pont-Saint-Martin,Quart,Gressan,Saint-Christophe,Courmayeur,Nus,Ayas,Gressoney"
 {
 "i": 697,
 "max": 11,
 "m": 0,
 },
 "name": "Gourmanche",
 "min": 4,
 "d": "nic-GH",
 "b": "Fada N'gourma,Pama,Diapaga,Bogande,Kantchari,Matiacoali,Gayeri,Piela,Bilanga,Manni,Botou,Tambaga"
 {
 "i": 715,
 "max": 11,
 "m": 0,
 },
 "name": "Tikar",
 "min": 4,
 "d": "nic-GH",
 "b": "Bankim,Magba,Ngambe-Tikar,Yoko,Linte,Ngoume,Ina,Foumban,Malentouen,Massangam,Kimi,Tibati"
 {
 "i": 717,
 "max": 11,
 "m": 0,
 },
 "name": "Twi",
 "min": 4,
 "d": "nic-GH",
 "b": "Kumasi,Obuasi,Mampong,Konongo,Ejisu,Bekwai,Akim Oda,Koforidua,Nkawkaw,Sunyani,Techiman,Wenchi"
 {
 "i": 728,
 "max": 11,
 "m": 0,
 },
 "name": "Azd dialect",
 "min": 4,
 "d": "nic-GH",
 "b": "Nizwa,Bahla,Ibri,Yanqul,Rustaq,Nakhal,Awabi,Samail,Bidbid,Izki,Adam,Manah"
 {
 "i": 738,
 "max": 11,
 "m": 0,
 },
 "name": "Asele",
 "min": 4,
 "d": "nic-GH",
 "b": "Asele,Dorotea,Vilhelmina,Storuman,Sorsele,Lycksele,Mala,Norsjo,Robertsfors,Vindeln,Vannas,Bjurholm"
 {
 "i": 741,
 "max": 11,
 "m": 0,
 },
 "name": "Achagua",
 "min": 4,
 "d": "nic-GH",
 "b": "Puerto Gaitan,Puerto Carreno,Inirida,Mitu,San Jose del Guaviare,Villavicencio,Yopal,Arauca,Leticia,Florencia,Mocoa,Popayan"
 {
 "i": 746,
 "max": 11,
 "m": 0,
 },
 "name": "Adang",
 "min": 4,
 "d": "nic-GH",
 "b": "Kalabahi,Alor,Pantar,Baranusa,Kabir,Marica,Mainang,Wolwal,Mataru,Kui,Abui,Kamang"
 {
 "i": 750,
 "max": 11,
 "m": 0,
 },
 "name": "Afade language",
 "min": 4,
 "d": "nic-GH",
 "b": "Afade,Kousseri,Makari,Goulfey,Blangoua,Fotokol,Logone-Birni,Waza,Mora,Meri,Bogo,Maroua"
 {
 "i": 762,
 "max": 11,
 "m": 0,
 },
 "name": "Aghu",
 "min": 4,
 "d": "nic-GH",
 "b": "Tanah Merah,Kouh,Bomakia,Mindiptana,Waropko,Yaniruma,Binamzain,Kawagit,Kombut,Wanggemalo,Edera,Venaha"
 {
 "i": 768,
 "max": 11,
 "m": 0,
 },
 "name": "Nonuya",
 "min": 4,
 "d": "nic-GH",
 "b": "Putumayo,Ica,Pasto,Santo Antonio do Ica,Guamues,Guamues River,San Miguel,Gueppa,Cumpuya,Algodon,Igara Parana,Cara Parana,Purete,Cotuhe,Solimu¡es,Japura"
 {
 "i": 780,
 "max": 11,
 "m": 0,
 },
 "name": "Tacana",
 "min": 4,
 "d": "nic-GH",
 "b": "Ixiamas,San Buenaventura,Abel Iturralde,Iturralde,Madidi,Beni,Beni River,Madre de Dios,Heath River,Franz Tamayo,Pando,Tambopata"
 {
 "i": 783,
 "max": 11,
 "m": 0,
 },
 "name": "Wiwa",
 "min": 4,
 "d": "nic-GH",
 "b": "Sierra Nevada de Santa Marta,La Guajira,Magdalena,Cesar,Santa Marta,Rancheraa,Don Diego,Palomino,Buritaca,Guachaca,Cienaga Grande de Santa Marta,Ariguana,Fundacion,Frao,Aracataca"
 {
 "i": 786,
 "max": 11,
 "m": 0,
 },
 "name": "Aimele",
 "min": 4,
 "d": "nic-GH",
 "b": "Nomad,Mougulu,Yehebi,Honinabi,Oksapmin,Telefomin,Kiunga,Tabubil,Ningerum,Olsobip,Gobe,Kantobo"
 {
 "i": 789,
 "max": 11,
 "m": 0,
 },
 "name": "Nataoran",
 "min": 4,
 "d": "nic-GH",
 "b": "Hualien,Ji'an,Shoufeng,Fenglin,Guangfu,Ruisui,Yuli,Fuli,Zhuoxi,Wanrong,Xincheng,Xiulin"
 {
 "i": 793,
 "max": 11,
 "m": 0,
 },
 "name": "Arafundi-Enga Pidgin",
 "min": 4,
 "d": "nic-GH",
 "b": "Wabag,Wapenamanda,Kandep,Laiagam,Porgera,Ambum,Kompiam,Maramuni,Paiela,Tsak,Wakumare,Sirunki"
 {
 "i": 804,
 "max": 11,
 "m": 0,
 },
 "name": "Akoye",
 "min": 4,
 "d": "nic-GH",
 "b": "Kerema,Malalaua,Ihu,Baimuru,Kikori,Purari,Gulf,Papua,Port Moresby,Lae,Madang,Wewak"
 {
 "i": 860,
 "max": 11,
 "m": 0,
 },
 "name": "Aluku",
 "min": 4,
 "d": "nic-GH",
 "b": "Paramaribo,NieuwNickerie,Albina,Moengo,Wageningen,Marienburg,Lelydorp,Brokopondo,Groningen,Zanderij,Brownsweg,Onverwacht"
 {
 "i": 890,
 "max": 11,
 "m": 0,
 },
 "name": "Auyokawa language",
 "min": 4,
 "d": "nic-GH",
 "b": "Kano,Katsina,Dutse,Hadejia,Ringim,Gwarzo,Rano,TudunWada,Gaya,Albasu,Danbatta,Makoda"
 {
 "i": 895,
 "max": 11,
 "m": 0,
 },
 "name": "Beele language",
 "min": 4,
 "d": "nic-GH",
 "b": "Bauchi,Ningi,Dass,Toro,Jamaa,Darazo,Misau,Katagum,Shira,Zaki,Giade,Gamawa"
 {
 "i": 906,
 "max": 11,
 "m": 0,
 },
 "name": "Voro",
 "min": 4,
 "d": "nic-GH",
 "b": "Tartu,Voru,Polva,Valga,Otepaa,Elva,Torva,KarksiNuia,Antsla,Rouge,Kanepi,Moniste"
 {
 "i": 911,
 "max": 11,
 "m": 0,
 },
 "name": "Aneme-Wake",
 "min": 4,
 "d": "nic-GH",
 "b": "PortMoresby,Lae,Madang,MountHagen,Goroka,Kokopo,Rabaul,Wewak,Popondetta,Alotau,Kavieng,Kimbe"
 {
 "i": 915,
 "max": 11,
 "m": 0,
 },
 "name": "isl",
 "min": 4,
 "d": "nic-GH",
 "b": "Reykjavik,Kopavogur,Hafnarfjordur,Akureyri,Reykjanesbaer,Selfoss,Akranes,Isafjordur,Egilsstadir,Husavik,Saudarkrokur,Vik,Gardabaer,Mosfellsbaer,Seltjarnarnes,Vestmannaeyjar,Grindavik,ilftanes,Hveragerdi,Sandgerdi,Dalvik,Seydisfjordur,Fjardabyggd,Flateyri"
 {
 "i": 923,
 "max": 11,
 "m": 0,
 },
 "name": "Arin",
 "min": 4,
 "d": "nic-GH",
 "b": "Moscow,SaintPetersburg,Novosibirsk,Yekaterinburg,NizhnyNovgorod,Kazan,Chelyabinsk,Omsk,Samara,RostovonDon,Ufa,Krasnoyarsk"
 {
 "i": 950,
 "max": 11,
 "m": 0,
 },
 "name": "Assan",
 "min": 4,
 "d": "nic-GH",
 "b": "Irkutsk,UlanUde,Chita,Bratsk,Angarsk,UsolyeSibirskoye,UstIlimsk,Shelekhov,Severobaikalsk,Tayshet,Bodaybo,Zima"
 {
 "i": 955,
 "max": 11,
 "m": 0,
 },
 "name": "Ekoka !Kung",
 "min": 4,
 "d": "nic-GH",
 "b": "Luanda,Huambo,Lubango,Benguela,Kuito,Malanje,Namibe,Soyo,Cabinda,Sumbe,Ondjiva,Menongue"
 {
 "i": 967,
 "max": 11,
 "m": 0,
 },
 "name": "Awjila language",
 "min": 4,
 "d": "nic-GH",
 "b": "Tripoli,Benghazi,Misurata,Zawiya,AlBayda,Tobruk,Ajdabiya,Sabha,Sirte,Derna,Khums,Zuwarah"
 {
 "i": 1007,
 "max": 11,
 "m": 0,
 },
 "name": "Barai",
 "min": 4,
 "d": "nic-GH",
 "b": "PortMoresby,Lae,Morobe,MountHagen,Arawa,Bougainville,Madang,Wewak,Goroka,Kokopo,Popondetta,Aitape"
 {
 "i": 1027,
 "max": 11,
 "m": 0,
 },
 "name": "Beni Snous dialect",
 "min": 4,
 "d": "nic-GH",
 "b": "Algiers,Oran,Constantine,Annaba,Blida,Batna,Djelfa,Setif,SidiBelAbbes,Biskra,Tebessa,Skikda"
 {
 "i": 1033,
 "max": 11,
 "m": 0,
 },
 "name": "Macaguan (Hitnu)",
 "min": 4,
 "d": "nic-GH",
 "b": "Bogota,Medellin,Cali,Barranquilla,Cartagena,Cucuta,Soledad,Ibague,Bucaramanga,Soacha,Villavicencio"
 {
 "i": 1035,
 "max": 11,
 "m": 0,
 },
 "name": "Muinane",
 "min": 4,
 "d": "nic-GH",
 "b": "Cali,Buga,Buenaventura,Palmira,Tulua,Jamundi,Cartago,Guadalajara,Pasto,Ipiales"
 {
 "i": 1038,
 "max": 11,
 "m": 0,
 },
 "name": "Satere-Mawe",
 "min": 4,
 "d": "nic-GH",
 "b": "SaoPaulo,RioDeJaneiro,BeloHorizonte,Brasilia,Salvador,Fortaleza,Recife,PortoAlegre,Manaus,Belem,Goiania,Campinas"
 {
 "i": 1040,
 "max": 11,
 "m": 0,
 },
 "name": "Warizu",
 "min": 4,
 "d": "nic-GH",
 "b": "SantaCruz,ElAlto,LaPaz,Cochabamba,Oruro,Sucre,Tarija,Potosi,Sacaba,Quillacollo,Montero,Trinidad"
 {
 "i": 1042,
 "max": 11,
 "m": 0,
 },
 "name": "Sanoma (Sanuma)",
 "min": 4,
 "d": "nic-GH",
 "b": "Caura,Auaris,Padamo,Mavaca,Demini,Parima,Watoriki,DeminiVillage,Balau,Hemosh,Toototobi,AuarisVillage"
 {
 "i": 1044,
 "max": 11,
 "m": 0,
 },
 "name": "Reyesano (Maropa)",
 "min": 4,
 "d": "nic-GH",
 "b": "Yumuruma,MaropaPara,Chirigua,Arapujira,MoxosPampas,YacumaLlano,Chirugua,Baritu,SanLorenzoMaropa,ReyesViejo,SantaAnaYacuma,CachuelaEsperanza"
 {
 "i": 1046,
 "max": 11,
 "m": 0,
 },
 "name": "Wichœa Lhamtes Nocten (Weenhayek)",
 "min": 4,
 "d": "nic-GH",
 "b": "Villa Montes,Nuestra Senora de la Paz,Tartagal,General Pinedo,Embarcacion,San Ramon,Yacuiba,Bermejo,Monteagudo,Rivadavia"
 {
 "i": 1048,
 "max": 11,
 "m": 0,
 },
 "name": "Sierra Popoluca",
 "min": 4,
 "d": "nic-GH",
 "b": "Soteapan,Tatahuicapa,San Pedro Soteapan,Jalapa,Cuitlapan,Popoloca,Tlacolula,Mazatlan"
 {
 "i": 1050,
 "max": 11,
 "m": 0,
 },
 "name": "Paipai",
 "min": 4,
 "d": "nic-GH",
 "b": "Santa Catarina,Quitovac,San Miguel,Mision San Javier,Chihuahua,San Ignacio,Terry,Cucurpe,Caborca,Pitiquito"
 {
 "i": 1052,
 "max": 11,
 "m": 0,
 },
 "name": "Tisza-Koros",
 "min": 4,
 "d": "nic-GH",
 "b": "Szeged,Kecskemet,Szolnok,Jaszbereny,Karcag,MezÅ‘tur,Szentes,Kunszentmarton,Turkeve,Kiskunfelegyhaza"
 {
 "i": 1070,
 "max": 11,
 "m": 0,
 },
 "name": "Peranpohjola",
 "min": 4,
 "d": "nic-GH",
 "b": "Perapohjola,Oulu,Kemi,Kokkola,Rovaniemi,Kemijärvi,Simo,Tornio,Haparanda,Pajala,Kilpisjarvi"
 {
 "i": 1097,
 "max": 11,
 "m": 0,
 },
 "name": "Crow",
 "min": 4,
 "d": "nic-GH",
 "b": "Billings,Lodge Grass,Big Horn,Crow Agency,Pryor,Hardin,Laurel,Busby,Bighorn,St Xavier,Rosebud,Lame Deer"
 {
 "i": 1108,
 "max": 11,
 "m": 0,
 },
 "name": "Harari",
 "min": 4,
 "d": "nic-GH",
 "b": "Harari,Ngaoundere,Gobir,Kargari,Yagana,Bodi,Koumbia,Yalogo,Ngourma,Bassi,Tchad,Doba"
 {
 "i": 1115,
 "max": 11,
 "m": 0,
 },
 "name": "Bana language",
 "min": 4,
 "d": "nic-GH",
 "b": "Bana,Garoua,Mokolo,Bamessing,Nsimey,Tignere,Nkamenbekoma,Guidar,Bantadje,Koumoulou,Mbe"
 {
 "i": 1124,
 "max": 11,
 "m": 0,
 },
 "name": "Bariji",
 "min": 4,
 "d": "nic-GH",
 "b": "Bariji,Papua New Guinea,Kundiawa,Mendi,Murua,Ialibu,Porgera,Mount Hagen,Goroka,Wabag,Lae"
 {
 "i": 1130,
 "max": 11,
 "m": 0,
 },
 "name": "Baruga",
 "min": 4,
 "d": "nic-GH",
 "b": "Baruga,Papua New Guinea,Oro Province,Ioma,Kokoda,Gona,Buna,Samarai,Popondetta,Morobe"
 {
 "i": 1138,
 "max": 11,
 "m": 0,
 },
 "name": "Bayono",
 "min": 4,
 "d": "nic-GH",
 "b": "Bayono,Awin,Bonua,Mandobo,Fai,Wanggom,Edolo,Kapori,Kasua,Munggona,Wano,Samberigi"
 {
 "i": 1160,
 "max": 11,
 "m": 0,
 },
 "name": "Lingua Geral Amazonica",
 "min": 4,
 "d": "nic-GH",
 "b": "Belem,Santarem,Manaus,Macapa,Boa Vista,Rio Branco,Porto Velho,Manacapuru,Parintins,Itacoatiara,Coari,Tefe"
 {
 "i": 1183,
 "max": 11,
 "m": 0,
 },
 "name": "Ixcatec",
 "min": 4,
 "d": "nic-GH",
 "b": "Santa Maria Ixcatlan,Asuncion Nochixtlan,Tilantongo,Yanhuitlan,Teposcolula,Chalcatongo de Hidalgo,Tlaxiaco,Huajuapan de Leon,Santiago Juxtlahuaca,San Pedro Tidaa,San Pedro Amuzgos,San Miguel Huautla"
 {
 "i": 1189,
 "max": 11,
 "m": 0,
 },
 "name": "Eyak",
 "min": 4,
 "d": "nic-GH",
 "b": "Eyak,Cordova,Bering River,Copper River,Malaspina Glacier,Montague Island,Hinchinbrook Island,Prince William Sound,Scott Glacier,Columbian Glacier,Meadow Glacier,Heiden Glacier"
 {
 "i": 1191,
 "max": 11,
 "m": 0,
 },
 "name": "Gwichin",
 "min": 4,
 "d": "nic-GH",
 "b": "Fort Yukon,Chalkytsik,Eagle Circle,Circle,Chicken,Fort McPherson,Tsiigehtchic,Inuvik,Aklavik,Tuktoyaktuk,Old Crow,West Dawson"
 {
 "i": 1195,
 "max": 11,
 "m": 0,
 },
 "name": "Holikachuk",
 "min": 4,
 "d": "nic-GH",
 "b": "Holy Cross,Holikachuk,Anvik,Grayling,Shageluk,Ruby,Kaltag,Nulato,Koyukuk,Galena,Huslia,Hughes"
 {
 "i": 1197,
 "max": 11,
 "m": 0,
 },
 "name": "Tlicho",
 "min": 4,
 "d": "nic-GH",
 "b": "Rae Behchoko,Whati,Gameti,Wekweeti,Dettah,Enterprise,Lutselk'e,Fort Smith,Yellowknife,Inuvik,Hay River,Norman Wells"
 {
 "i": 1199,
 "max": 11,
 "m": 0,
 },
 "name": "Upper Tanana",
 "min": 4,
 "d": "nic-GH",
 "b": "Northway,Tetlin,Tok,Eagle,Chicken,Dawson City,Whitehorse,Carcross,Teslin,Atlin,Skagway,Haines Junction"
 {
 "i": 1201,
 "max": 11,
 "m": 0,
 },
 "name": "Quechan",
 "min": 4,
 "d": "nic-GH",
 "b": "Yuma,Somerton,San Luis,Wellton,Winterhaven,Fort Yuma,Quartermaster Landing,Pilot Knob,Madrid,Ogilby,Dateland,Arlington"
 {
 "i": 1206,
 "max": 11,
 "m": 0,
 },
 "name": "Oneida",
 "min": 4,
 "d": "nic-GH",
 "b": "Oneida,Green Bay,Syracuse,Rome,Oneida Castle,Vernon,Stockbridge,Albany,De Pere,Appleton,Shawano,Manitowoc"
 {
 "i": 1213,
 "max": 11,
 "m": 0,
 },
 "name": "Cubeo",
 "min": 4,
 "d": "nic-GH",
 "b": "Cubeo,Vaupes,Cabuari,Piratapuyo,Tiquie,Papuri,Uaupes,Querari,Canuma,Pira-Parana,Yapura,Icana"
 {
 "i": 1215,
 "max": 11,
 "m": 0,
 },
 "name": "Itene",
 "min": 4,
 "d": "nic-GH",
 "b": "Itene,Beni,Mamore,Madre de Dios,Orton,Puerto Rico,Reyes,Rurrenabaque,San Borja,San Ignacio,Yacuma,Itonomas"
 {
 "i": 1217,
 "max": 11,
 "m": 0,
 },
 "name": "Koreguaje",
 "min": 4,
 "d": "nic-GH",
 "b": "Koreguaje,Orteguaza,Putumayo,Caqueta,Mocoa,Villagarzon,Sibundoy,Puerto Leguizamo,San Miguel,La Tagua,Puerto Asis,Puerto Guzman"
 {
 "i": 1219,
 "max": 11,
 "m": 0,
 },
 "name": "Wanano",
 "min": 4,
 "d": "nic-GH",
 "b": "Wanano,Sao Gabriel da Cachoeira,Uaupes,Papuri,Tiquie,Icana,Querari,Cari,Caruru,Miriti-Piraparana,Yaupura,Apaporis"
 {
 "i": 1221,
 "max": 11,
 "m": 0,
 },
 "name": "Siriano",
 "min": 4,
 "d": "nic-GH",
 "b": "Siriano,Siena,Poggibonsi,San Gimignano,Colle Val d'Elsa,Montalcinato,Radicofani,Buonconvento,Firenze,Prato,Empoli"
 {
 "i": 1224,
 "max": 11,
 "m": 0,
 },
 "name": "Bee",
 "min": 4,
 "d": "nic-GH",
 "b": "Bee,Beijing,Hangzhou,Xianyang,Shijiazhuang,Tianjin,Chengde,Qinhuangdao,Shijingshan,Chifeng"
 {
 "i": 1229,
 "max": 11,
 "m": 0,
 },
 "name": "Belneng language",
 "min": 4,
 "d": "nic-GH",
 "b": "Belneng,Papua New Guinea,Enga,Mendi,Mount Hagen,Goroka,Wabag,Lae,Port Moresby,Kokopo,Rabaul,Wau"
 {
 "i": 1239,
 "max": 11,
 "m": 0,
 },
 "name": "Kiowa",
 "min": 4,
 "d": "nic-GH",
 "b": "Anadarko,Lawton,Meers,Gotebo,Hobart,Cache,Snyder,Carnegie,Apache,Geronimo,SugarCreek,RainyMtn"
 {
 "i": 1279,
 "max": 11,
 "m": 0,
 },
 "name": "Biangai",
 "min": 4,
 "d": "nic-GH",
 "b": "Biangai,Morobe,Wau,Bulolo,Lae,Popondetta,Kokopo,Rabaul,Madang,Mount Hagen,Goroka,Port Moresby"
 {
 "i": 1282,
 "max": 11,
 "m": 0,
 },
 "name": "Piaroa",
 "min": 4,
 "d": "nic-GH",
 "b": "Piaroa,Puerto Ayacucho,San Fernando de Atabapo,San Carlos de Rio Negro,El Paujil,Mitu,Tapirapeco,Maquiritare,Yavita,Tama,Manapiare,Vichada"
 {
 "i": 1284,
 "max": 11,
 "m": 0,
 },
 "name": "Mikasuki",
 "min": 4,
 "d": "nic-GH",
 "b": "Mikasuki,Immokalee,Brighton,Big Cypress,Hollywood,Tampa,Miami,Reservation,Lee County,Collier County,Glades County,Hendry County"
 {
 "i": 1298,
 "max": 11,
 "m": 0,
 },
 "name": "Bara",
 "min": 4,
 "d": "nic-GH",
 "b": "Catatumbo,Tibu,LaGabarra,Sardinata,Convencion,ElTarra,Ocaœea,Teorama,Hacara,SanCalixto,LaPlaya,Abrego"
 {
 "i": 1305,
 "max": 11,
 "m": 0,
 },
 "name": "Binahari",
 "min": 4,
 "d": "nic-GH",
 "b": "Binahari,Gulf Province,Port Moresby,Kikori,Baimuru,Kerema,Malalaua,Morobe,Madang,Papua New Guinea"
 {
 "i": 1307,
 "max": 11,
 "m": 0,
 },
 "name": "Binumarien",
 "min": 4,
 "d": "nic-GH",
 "b": "Binumarien,Morobe Province,Lae,Bulolo,Menyamya,Kainantu,Wau,Finschhafen,Madang,Papua New Guinea"
 {
 "i": 1310,
 "max": 11,
 "m": 0,
 },
 "name": "Yuracare",
 "min": 4,
 "d": "nic-GH",
 "b": "Yuracare,San Borja,Trinidad,San Javier,Mamore,Beni River,Chimore,Santa Cruz de la Sierra,Amazon Basin,Bolivia"
 {
 "i": 1312,
 "max": 11,
 "m": 0,
 },
 "name": "Boghom language",
 "min": 4,
 "d": "nic-GH",
 "b": "Boghom,Nigeria,Missau,Jama'are,Bauchi,Potiskum,Dambam,Kirfi,Bi,Shira,Yashi,Zari"
 {
 "i": 1315,
 "max": 11,
 "m": 0,
 },
 "name": "Bole Chadic language",
 "min": 4,
 "d": "cdc-NG",
 "b": "Fika,Potiskum,Geidam,Damaturu,Gashua,Nguru,Hadejia,Azare,Misau,Jama'are,Katagum,Kirfi,Shira,Yashi,Zari,Kariya,Boghom,Mahamuda,Warji,Saya,Ningi,Alkaleri,Tafawa Balewa"
 {
 "i": 1318,
 "max": 11,
 "m": 0,
 },
 "name": "Paez",
 "min": 4,
 "d": "nic-GH",
 "b": "Paez,Inza,Purace,Totoro,Jambalo,El Tambo,Piendamo,Silvia,Popayan,Cauca,Colombia"
 {
 "i": 1321,
 "max": 11,
 "m": 0,
 },
 "name": "Bisorio",
 "min": 4,
 "d": "nic-GH",
 "b": "Bisorio,Papua New Guinea,Enga,Mendi,Kandep,Wabag,Laiagam,Mount Hagen,Porgera,Wapenamanda,Tsaka,Kar"
 {
 "i": 1327,
 "max": 11,
 "m": 0,
 },
 "name": "Bjarmian Sami",
 "min": 4,
 "d": "nic-GH",
 "b": "Bjarmian,Sami,Kautokeino,Karasjok,Alta,Tromso,Hammerfest,Northern Norway,Finnmark,Norway,Sapmi"
 {
 "i": 1329,
 "max": 11,
 "m": 0,
 },
 "name": "Bintulu",
 "min": 4,
 "d": "nic-GH",
 "b": "Bintulu,Miri,Sibu,Bintulu Division,Sebiew,Sungai Plan,Bakong,Song,Sibuti,Tatau,Sarawak,Borneo"
 {
 "i": 1332,
 "max": 11,
 "m": 0,
 },
 "name": "Boazi",
 "min": 4,
 "d": "nic-GH",
 "b": "Boazi,Papua New Guinea,Kubor,Kagua,Erave,Ialibu,Pangia,Mendi,Mount Hagen,Porgera,Western Highlands,Karamui"
 {
 "i": 1334,
 "max": 11,
 "m": 0,
 },
 "name": "Fulnio",
 "min": 4,
 "d": "nic-GH",
 "b": "Fulni-o,iguas Belas,Palmares,Pedra,Ouricuri,Caruaru,Recife,Pernambuco,Brazil"
 {
 "i": 1337,
 "max": 11,
 "m": 0,
 },
 "name": "Leco",
 "min": 4,
 "d": "nic-GH",
 "b": "Leco,Apolo,Apolo Province,La Paz,Bolivia,Amazon Basin,Madre de Dios,Peru,Beni River"
 {
 "i": 1343,
 "max": 11,
 "m": 0,
 },
 "name": "Bonan Manegacha lect",
 "min": 4,
 "d": "nic-GH",
 "b": "Bonan Manegacha,Gansu,Qinghai,Xiahe,Minhe,Xunhua,Tongren,Haibei,Guoluo,Huangnan"
 {
 "i": 1364,
 "max": 11,
 "m": 0,
 },
 "name": "Shoshoni",
 "min": 4,
 "d": "nic-GH",
 "b": "Shoshoni,Wind River,Fort Hall,Duck Valley,Elko,Ely,Wells,Wendover,Twin Falls,Idaho Falls,Jackpot,Elko County"
 {
 "i": 1379,
 "max": 11,
 "m": 0,
 },
 "name": "Broome Pearling Lugger Pidgin",
 "min": 4,
 "d": "nic-GH",
 "b": "Broome,Cable Beach,Roebuck Bay,Gantheaume Point,Derby,Fitzroy River,Kimberley,Western Australia,Australia,Pearl Luggers"
 {
 "i": 1387,
 "max": 11,
 "m": 0,
 },
 "name": "Enlhet",
 "min": 4,
 "d": "nic-GH",
 "b": "Enlhet,Paraguay,Alto Paraguay,Carmelo Peralta,Filadelfia,Boqueron,Mariscal Estigarribia,Gran Chaco,Paraguayan Chaco"
 {
 "i": 1395,
 "max": 11,
 "m": 0,
 },
 "name": "Wounaan",
 "min": 4,
 "d": "nic-GH",
 "b": "Wounaan,Panama,Darien Province,Sambu,Cucunati,Yaviza,Puerto Pilon,Real,Carti,Tuira River,Chepo River"
 {
 "i": 1400,
 "max": 11,
 "m": 0,
 },
 "name": "Burumakok",
 "min": 4,
 "d": "nic-GH",
 "b": "Burumakok,Papua New Guinea,Sandaun Province,Wewak,Aitape,Lae,Madang,Ramu River,Sepik River,East Sepik"
 {
 "i": 1402,
 "max": 11,
 "m": 0,
 },
 "name": "Kalaallisut",
 "min": 4,
 "d": "nic-GH",
 "b": "Kalaallisut,Nuuk,Ilulissat,Paamiut,Maniitsoq,Qaqortoq,Sisimiut,Aasiaat,Nuussuaq,Greenland,Kalaallit Nunaat"
 {
 "i": 1408,
 "max": 11,
 "m": 0,
 },
 "name": "Carijona",
 "min": 4,
 "d": "nic-GH",
 "b": "Carijona,Araracuara,Yauarete,Sao Gabriel da Cachoeira,Uaupes,Papuri,Cabucuri,Caruru,Tiquie,Icana,Negro River"
 {
 "i": 1410,
 "max": 11,
 "m": 0,
 },
 "name": "Cuiba",
 "min": 4,
 "d": "nic-GH",
 "b": "Cuiba,Villavicencio,Meta,Puerto Gaitan,Orocue,Puerto Lleras,Casanare,Colombia,Orinoco River,Llanos Orientales"
 {
 "i": 1415,
 "max": 11,
 "m": 0,
 },
 "name": "Cahuilla",
 "min": 4,
 "d": "nic-GH",
 "b": "Cahuilla,Palm Springs,Palm Desert,Indio,Coachella,Desert Hot Springs,Thousand Palms,Rancho Mirage,California"
 {
 "i": 1420,
 "max": 11,
 "m": 0,
 },
 "name": "Cayuvava",
 "min": 4,
 "d": "nic-GH",
 "b": "Cayuvava,Santa Cruz,Trinidad,San Javier,San Ramon,Beni,Bolivia,Amazon Basin,Mamore River"
 {
 "i": 1422,
 "max": 11,
 "m": 0,
 },
 "name": "Guahibo (Sikuani)",
 "min": 4,
 "d": "nic-GH",
 "b": "Guahibo,Sikuani,Puerto Gaitan,Orocue,Puerto Carreno,Vichada,Casanare,Meta,Colombia,Orinoco River,Llanos Orientales"
 {
 "i": 1425,
 "max": 11,
 "m": 0,
 },
 "name": "Guaji",
 "min": 4,
 "d": "nic-GH",
 "b": "Guaji,El Carrizal,Maucara,Socorro,La Palma,San Martin,San Cristobal,Casanare,Santander,Colombia,Venezuela"
 {
 "i": 1427,
 "max": 11,
 "m": 0,
 },
 "name": "Inuinnaqtun",
 "min": 4,
 "d": "nic-GH",
 "b": "Inuinnaqtun,Igloolik,Pond Inlet,Rankin Inlet,Baker Lake,Chesterfield Inlet,Nunavut,Canada,Arctic,Coppermine"
 {
 "i": 1434,
 "max": 11,
 "m": 0,
 "b": "Carolinian,Saipan,Tinian,Rota,Caroline Islands,Pacific,Micronesia,Northern Mariana Islands"
 },
 {
 "name": "Lacandon",
 "i": 1444,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lacandon,Lacandona Jungle,Chiapas,Mexico,Tabasco,Guatemala,Peten,Rain Forest,Usumacinta River"
 },
 {
 "name": "Poqomam",
 "i": 1447,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Poqomam,Quiche,Guatemala,Quiche Department,Maya Highlands,Mesoamerica,El Quiche,Guatemala City"
 },
 {
 "name": "Poqomchi'",
 "i": 1448,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Poqomchi',Quiche,Guatemala,Maya Highlands,Chichicastenango,Solola,Quiche Department"
 },
 {
 "name": "Tojolab'al",
 "i": 1449,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tojolab'al,Huehuetenango,Guatemala,Nebaj,Chalchitan,Soloma,Jalapa,Quiche,Maya Highlands"
 },
 {
 "name": "Tz'utujil",
 "i": 1450,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tz'utujil,Quiche,Guatemala,Quiche Department,Maya Highlands,Chajul,Cotzal,Nebaj"
 },
 {
 "name": "Sakapultek",
 "i": 1452,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sakapultek,Guatemala,Sacapulas,Santa Rosa,El Progreso,San Benito,Ciudad Hidalgo,Quiche"
 },
 {
 "name": "Sipakapense",
 "i": 1453,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sipakapense,Sipakapa,Santa Cruz de Yojoa,Copan,Honduras,El Paraiso,San Pedro Sula,Olancho"
 },
 {
 "name": "Tektitek",
 "i": 1454,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tektitek,Guatemala,San Cristobal Acasaguastlan,Santa Rosa de Lima,San Marcos,Cuatemala"
 },
 {
 "name": "Uspanteko",
 "i": 1455,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Uspanteko,Guatemala,Usulatan,Zacapa,Chiquimula,Izabal,Alta Verapaz,Guatemala City"
 },
 {
 "name": "Francais Tirailleur",
 "i": 1485,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "French Tirailleur,Dakar,St. Louis,Senegal,Mali,Burkina Faso,Ivory Coast,Guinea,Niger,West Africa"
 },
 {
 "name": "Matlatzinca",
 "i": 1497,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Matlatzinca,Toluca,Almoloya de Juarez,Calimaya,Metepec,San Mateo Atenco,Mexico,State of Mexico"
 },
 {
 "name": "Ginuman",
 "i": 1504,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ginuman,Papua New Guinea,Sepik,Ambunti,East Sepik,Wewak,Oceania"
 },
 {
 "name": "Glavda language",
 "i": 1507,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Glavda,Adamawa,Bama,Gwoza,Maiduguri,Nigeria,Cameroon,Chad Border"
 },
 {
 "name": "Tarahumara",
 "i": 1512,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tarahumara,Guachochi,Bocoyna,Creel,Chihuahua,Mexico,Copper Canyon,Sierra Madre Occidental"
 },
 {
 "name": "Huarijio",
 "i": 1513,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Huarijio,Alamos,El Fuerte,Sonora,Sinaloa,Mexico,Sierra Madre Occidental"
 },
 {
 "name": "Gobasi",
 "i": 1520,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gobasi,Western Province,Kiunga,Papua New Guinea,Fly River,Oceania,Coastal"
 },
 {
 "name": "Goemai language",
 "i": 1521,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goemai,Shendam,Langtang,Plateau State,Nigeria,Jos,West Africa"
 },
 {
 "name": "Goji language",
 "i": 1523,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goji,Mandara Plateau,Mokolo,Maroua,Cameroon,Nigeria Border"
 },
 {
 "name": "Chakato language",
 "i": 1536,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chakato,Kansas,Oklahoma,Missouri,USA,Mississippi River,Great Plains,Central Plains"
 },
 {
 "name": "Macushi",
 "i": 1545,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Macushi,Lethem,Rupununi,Mabaruma,Guyana,Brazil Border,Venezuela Border"
 },
 {
 "name": "Yukpa",
 "i": 1547,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Yukpa,Machiques,Venezuela,Colombia,Perija Mountains,Sierra de Perija,Andes Border"
 },
 {
 "name": "Fut",
 "i": 1548,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Futuna,Futuna Island,Wallis and Futuna,Territory,Pacific Ocean,Polynesia"
 },
 {
 "name": "Piapoco",
 "i": 1579,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Piapoco,Puerto Carreno,Orocue,Vichada,Colombia,Meta,Orinoco River,South America"
 },
 {
 "name": "Terena",
 "i": 1580,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Terena,Mato Grosso do Sul,Ponta Pora,Brazil,Paraguay Border,Guarani Region"
 },
 {
 "name": "Wapishana",
 "i": 1581,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Wapishana,Boa Vista,Roraima,Brazil,Amazon Rainforest,Guiana Highlands"
 },
 {
 "name": "Chimbu",
 "i": 1586,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chimbu,Kundiawa,Papua New Guinea,Chimbu Province,Highlands Region,Oceania"
 },
 {
 "name": "Chinook Jargon",
 "i": 1592,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chinook Jargon,Portland,Seattle,Vancouver,Columbia River,Oregon,Washington,USA,Canada,Pacific Northwest"
 },
 {
 "name": "Cholti Classic",
 "i": 1595,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cholti,Mixco,Antigua Guatemala,Guatemala,Central America,Maya Region"
 },
 {
 "name": "Chuave",
 "i": 1600,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chuave,Chuave District,Simbu Province,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Chorote",
 "i": 1603,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chorote,La Paz,Villamontes,Argentina,Bolivia,Gran Chaco,Paraguay Border"
 },
 {
 "name": "Chorotega",
 "i": 1604,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Chorotega,Guancaste,Nicoya,Liberia,Costa Rica,Managua,Granada,Nicaragua"
 },
 {
 "name": "Choshuenco",
 "i": 1605,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Choshuenco,Valdivia,Rio Bueno,Osorno,Los Lagos,Chile,Patagonia,Andes,Lake District"
 },
 {
 "name": "Cibak language",
 "i": 1613,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cibak,Bama,Mubi,Mubi North,Adamawa State,Nigeria,North East"
 },
 {
 "name": "Cineni language",
 "i": 1615,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cineni,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Ciwogai language",
 "i": 1618,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ciwogai,Mayo Belwa,Jalingo,Gongola State,Nigeria,North East,West Africa"
 },
 {
 "name": "Kavalan",
 "i": 1622,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kavalan,Su'ao,Yilan,Taiwan,East Coast,East Asia"
 },
 {
 "name": "Chicomuceltec",
 "i": 1630,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Comitan,Trinitaria,La Independencia,Francisco Leon,Socoltenango,Huehuetenango,Guatemala,Maya Region,Central America"
 },
 {
 "name": "Lonwolwol",
 "i": 1641,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lonwolwol,Santo,Port Vila,Vanuatu,Oceania,South Pacific"
 },
 {
 "name": "Cuvok language",
 "i": 1646,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cuvok,Mokolo,Maroua,Cameroon,Mandara Mountains,West Africa"
 },
 {
 "name": "Daga",
 "i": 1650,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Daga,Mendi,Mount Hagen,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Dano",
 "i": 1656,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dano,Mendi,Mount Hagen,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Dap",
 "i": 1658,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dap,Mount Hagen,Kundiawa,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Dass language",
 "i": 1662,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dass,Bauchi,Plateau State,Nigeria,North Central,West Africa"
 },
 {
 "name": "Dazawa language",
 "i": 1664,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dazawa,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Deno language",
 "i": 1669,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Deno,Kisangani,Bunia,DR Congo,Central Africa,Lake Albert"
 },
 {
 "name": "Densar",
 "i": 1670,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Densar,Kisangani,Bukavu,DR Congo,Central Africa,Lake Kivu"
 },
 {
"name": "Sened",
 "i": 1686,
 "min": 4,
 "max": 11,
 "d": "ber-TN",
 "m": 0,
 "b": "Sened,Gafsa,Tozeur,Kebili,Tamerza,Mides,Chebika,Nefta,El Hamma,Metlaoui,Redeyef,Bir el Hafey,Maknassy,Sidi Bouzid"
 },
 {
 "name": "Dghwede language",
 "i": 1697,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dghwede,Gwoza,Bama,Bornu State,Nigeria,North East,West Africa"
 },
 {
 "name": "Diri language",
 "i": 1702,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Diri,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Douiret language",
 "i": 1716,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Douiret,Beni Mellal,Fes,Morocco,Atlas Mountains,North Africa"
 },
 {
 "name": "Dugwor language",
 "i": 1720,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dugwor,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Duhwa language",
 "i": 1721,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Duhwa,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Duna",
 "i": 1723,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Duna,Mount Hagen,Kundiawa,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Duvle Wano Pidgin",
 "i": 1733,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Duvle Wano Pidgin,Morobe Province,Lae,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Duwai language",
 "i": 1734,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Duwai,Mubi,Bama,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Edolo",
 "i": 1755,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Edolo,Edo State,Benin City,Nigeria,South South,Niger Delta,West Africa"
 },
 {
 "name": "Ekari",
 "i": 1757,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Enarotali,Sugapa,Ilaga,Paniai,Deiyai,Dogiyai,Moanemani,Waghete,Obano,Bibida,Tigi,Kaimana,Nabire,Timika,Wamena"
 },
 {
 "name": "Enga",
 "i": 1767,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Enga,Wabag,Lae,Papua New Guinea,Morobe Province,Oceania"
 },
 {
 "name": "Ese -mie",
 "i": 1772,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ese Omie,Kuopio,Haapajarvi,Oulu,Finland,Bothnian,Baltic Sea"
 },
 {
 "name": "Esimbi",
 "i": 1773,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Esimbi,Kuopio,Iisalmi,Kokkola,Finland,Bothnian,Baltic Sea"
 },
 {
 "name": "Eskimo Trade Jargon",
 "i": 1774,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Eskimo Trade Jargon,Arctic,Inuit,Greenland,Canada,North America,Alaska"
 },
 {
 "name": "Ewage",
 "i": 1778,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ewage,Morobe Province,Lae,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Faiwol",
 "i": 1781,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Faiwol,Enga Province,Wabag,Papua New Guinea,Western Highlands,Oceania"
 },
 {
 "name": "Fasu",
 "i": 1789,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Mendi,Mount Hagen,Kundiawa,Papua New Guinea,Western Highlands,Oceania"
 },
 {
 "name": "Fataluku",
 "i": 1790,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lospalos,Baucau,Tutuala,Fatularu,Iliomar,Ossu,Lautem,Timor-Leste,Timor Island,Oceania"
 },
 {
 "name": "Dom",
 "i": 1796,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Mubi,Bama,Gombi,Uba,Maiha,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Domu",
 "i": 1797,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Domu,Morobe Province,Bulolo,Lae,Wau,Papua New Guinea,Highlands Region,Oceania"
 },
 {
 "name": "Dima",
 "i": 1804,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Yola,Jalingo,Gongola,Muri,Mubi,Adamawa State,Nigeria,North East,West Africa"
 },
 {
 "name": "Doromu",
 "i": 1808,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Doromu,Kundiawa,Mawatobo,Yombi,Papua New Guinea,Chimbu Province,Eastern Highlands,Oceania"
 },
 {
 "name": "E (Austronesian)",
 "i": 1814,
 "min": 4,
 "max": 11,
 "d": "map-ID",
 "m": 0,
 "b": "Halmahera,Tobelo,Galela,Morotai,Kao,Jailolo,Ternate,Tidore,Bacan,Makian,Sanana,Mangole,Taliabu,Obi,Weda,Patani,Wasile,Maba"
 },
 {
 "name": "Favorlang",
 "i": 1820,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Favorlang,Taoyuan,Taipei,Hsinchu,Taiwan,Formosan,Changhua,Nantou,Zhanghua,China,Taiwan Strait"
 },
 {
 "name": "Golin",
 "i": 1821,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Simbu,Kundiawa,Mari-Mari,Gembogl,Papua New Guinea,Simbu Province,Highlands Region,Oceania"
 },
 {
 "name": "Goroka",
 "i": 1824,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goroka,Kundiawa,Mendi,Chuave,Papua New Guinea,Eastern Highlands,Simbu Province"
 },
 {
 "name": "Grass Koiari",
 "i": 1827,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Koiari,Kwikila,Kalana,Maopa,Papua New Guinea,Central Province,Oceania"
 },
 {
 "name": "Gauwa",
 "i": 1834,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gauwa,Andra,Pradiput,Simlipal,Odisha,India,Eastern India,Kalahandi Region"
 },
 {
 "name": "Gawar Language",
 "i": 1835,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gawar,Arunachal Pradesh,India,Northeast India,Himalayas,China Border"
 },
 {
 "name": "Fembe",
 "i": 1842,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fembe,Port Vila,Efate,Maevo,Santo,Tanna,Vanuatu,Oceania,South Pacific"
 },
 {
 "name": "Fio",
 "i": 1845,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fio,Mendi,Mount Hagen,Kundiawa,Papua New Guinea,Western Highlands,Oceania"
 },
 {
 "name": "Fiwaga",
 "i": 1846,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fiwaga,Wabag,Kagua,Tapinin,Papua New Guinea,Enga Province,Western Highlands,Oceania"
 },
 {
 "name": "Foe",
 "i": 1848,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Foe,Kurikupapa,Oro,Baimuru,Papua New Guinea,Gulf Province,Oceania"
 },
 {
 "name": "Foia Foia",
 "i": 1849,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Foia,Papua New Guinea,Kimbe,Walomo,Mendi,Western Highlands,Oceania"
 },
 {
 "name": "Fore",
 "i": 1851,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fore,Kukukuku,Sene,Poraro,Papua New Guinea,Simbu Province,Eastern Highlands,Oceania"
 },
 {
 "name": "Siraya",
 "i": 1853,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Siraya,Tainan,Tamsui,Taiwan,Sinckan,Banciao,Tsoyan,Taiwanese"
 },
 {
 "name": "Fuyug",
 "i": 1856,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fuyug,Mafulu,Orongomo,Managalasi,Iome,Papua New Guinea,Central Province,Oceania"
 },
 {
 "name": "Gällivare",
 "i": 1858,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gällivare,Kiruna,Malmberget,Norrbottn,Sweden,Arctic,Lapland,Sami Region"
 },
 {
 "name": "Gende",
 "i": 1864,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gende,Finisterre,Popondetta,Yam,Papua New Guinea,Madang Province,Oceania"
 },
 {
 "name": "Ghadames",
 "i": 1870,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ghadames,Ghat,Fezzan,Nalut,Algeria,Sahara Desert,North Africa"
 },
 {
 "name": "Gullah",
 "i": 1879,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Charleston,Savannah,Sea Islands,Georgetown,Beaufort,Low Country,Gullah Geechee,USA,Southeast Coast"
 },
 {
 "name": "Ghomara language",
 "i": 1893,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ghomara,Marrakech,El Hajeb,Ait Mellal,Taroudant,Tinrhir,Morocco,Atlas,North"
 },
 {
 "name": "Gidar language",
 "i": 1895,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gidar,Mubi,Bama,Pala,Kuje,Gujj,Bauchi,Adamawa,Nigeria,East"
 },
 {
 "name": "Huli",
 "i": 1902,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Huli,Tari,Iba,Muli,Png,Southern,Highlands,Papua New Guinea"
 },
 {
 "name": "Humene",
 "i": 1903,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Humene,Kwaia,Manubara,Png,Central,Province,Oceania"
 },
 {
 "name": "Hupla",
 "i": 1905,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Hupla,Dani,Baliem,Indonesia,Highlands,Papua,Papua New Guinea"
 },
 {
 "name": "Hwanghae Dialect",
 "i": 1908,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Hwanghae,Haeju,Kaesong,Hongchong,Chuncheon,North Korea,Eastern,Korea"
 },
 {
 "name": "Iha",
 "i": 1914,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Iha,Tidore,Halmahera,Papua New Guinea,West Sepik Province,Oceania"
 },
 {
 "name": "Giiwo language",
 "i": 1917,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Toro,Bauchi,Jos,Pankshin,Shendam,Langtang,Kanke,Mangu,Bokkos,Barkin Ladi,Riyom,Bassa,Wase,Kanam,Mikang"
 },
 {
 "name": "Gimi",
 "i": 1918,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gimi,Mendi,Mandj,Kundiawa,Chimbu,Papua New Guinea,Eastern Highlands"
 },
 {
 "name": "Guruntum language",
 "i": 1920,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Guruntum,Shelleng,Tel,Kambari,Adamawa State,Nigeria,North East"
 },
 {
 "name": "Gvoko language",
 "i": 1922,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gvoko,Gwoza,Mubi,Bama,Adamawa State,Nigeria,North East"
 },
 {
 "name": "Gwandara language",
 "i": 1924,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gwandara,Gwari,Kaduna,Kano,Birnin Kebbi,Kano State,Nigeria,West"
 },
 {
 "name": "Gyeonggi-Seoul Dialect",
 "i": 1929,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gyeonggi,Gyeonggi-Seoul,Seoul,Incheon,Suwon,Bucheon,South Korea,Province"
 },
 {
 "name": "Gyeongsang Dialect",
 "i": 1930,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gyeongsang,Busan,Ulsan,Tongyeongong,Changwon,Miryang,South Korea,Province"
 },
 {
 "name": "Hamtai",
 "i": 1931,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Hamtai,Warahuta,Mendeng,Kalam,Yani,Mendi,Papua New Guinea,Eastern Highlands"
 },
 {
 "name": "Armenian Dialect",
 "i": 1938,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Armenian,East,Syunik,Echmiadzin,Tavush,Masisis,Armenia,Caucasus"
 },
 {
 "name": "Inoke-Yate",
 "i": 1949,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goroka,Kainantu,Henganofi,Lufa,Okapa,Obura,Wonenara,Daulo,Watabung,Unggai"
 },
 {
 "name": "Inupiaq",
 "i": 1952,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Utqiaa¡vik,Kotzebue,Point Hope,Noorvik,Selawik,Wainwright,Kivalina,Buckland,Kiana,Ambler"
 },
 {
 "name": "Inuvialuktun",
 "i": 1953,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Inuvik,Tuktoyaktuk,Paulatuk,Sachs Harbour,Ulukhaktok,Aklavik,Tsiigehtchic,Fort McPherson,Old Crow,Dawson City"
 },
 {
 "name": "Puinave",
 "i": 1961,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Inirida,Barrancominas,Mapiripana,San Felipe,Puerto Colombia,Pana-Pana,Puerto Inirida,Guaviare,Vichada,Vaupes,Amazonas,Atabapo"
 },
 {
 "name": "Yanomami",
 "i": 1965,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Surucucu,Catrimani,Toototobi,Paapiu,Demini,Homoxi,Xidea,Erico,Auaris,Mavaca,Ocamo,Platanal"
 },
 {
 "name": "Sanoma",
 "i": 1966,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Auaris,Erico,Toototobi,Demini,Homoxi,Xidea,Catrimani,Surucucu,Mavaca,Ocamo,Platanal,Santa Isabel"
 },
 {
 "name": "Abinomn",
 "i": 1969,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Baso,Fofia,Kwerba,Apauwar,Sarmi,Bonggo,Tor,Mamberamo,Lereh,Genyem"
 },
 {
 "name": "Abun",
 "i": 1970,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sausapor,Werur,Bira,Wau,Syuru,Kwoor,Moraid,Madik,Fiawat,Makbon"
 },
 {
 "name": "Anem",
 "i": 1971,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kimbe,Hoskins,Bialla,Gloucester,Kandrian,Talasea,Gasmata,Pomio,Cape Gloucester,Arawe"
 },
 {
 "name": "Bilua",
 "i": 1972,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gizo,Munda,Ringgi,Noro,Barakoma,Vella Lavella,Kolombangara,Ranongga,Simbo,Choiseul"
 },
 {
 "name": "Kuot",
 "i": 1973,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kavieng,Namatanai,Konos,Lihir,Tabar,Tanga,Feni,New Hanover,Mussau,Emirau"
 },
 {
 "name": "Kamang",
 "i": 1999,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kalabahi,Apui,Mainang,Wolwal,Mataru,Kui,Kolana,Pureman,Erana,Alor"
 },
 {
 "name": "Kamasa",
 "i": 2000,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Morobe,Wau,Bulolo,Lae,Salamaua,Finschhafen,Kaiapit,Menyamya,Pindiu,Boana"
 },
 {
 "name": "Kamberau",
 "i": 2003,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kamberau,Arguni,Bedidi,Namatota,Kaimana,Etna Bay,Triton Bay,Lobo,Buruway,Teluk Arguni"
 },
 {
 "name": "Ron family",
 "i": 2011,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Bokkos,Mangu,Pankshin,Barkin Ladi,Jos,Riyom,Bassa,Jos East,Jos South,Jos North"
 },
 {
 "name": "Karas",
 "i": 2020,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Fakfak,Kaimana,Kokas,Bomberay,Karas Island,Teluk Patipi,Kramomongmega,Arguni,Buruway,Muri"
 },
 {
 "name": "Ikobi",
 "i": 2024,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Ikobi,Kairi,Omati,Kikori,Turama,Goaribari,Aird Hills,Baimuru,Purari,Ihu,Kerema,Malalaua,Kanabea,Kaintiba,Kamina,Wabo"
 },
 {
 "name": "Ipiko",
 "i": 2027,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Ipiko,Inland Gulf,Pie,Purari,Sirebi,Kivori,Iokea,Lese,Moveave,Tauri,Lakekamu,Vailala,Orokolo,Murua"
 },
 {
 "name": "Ipili",
 "i": 2028,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Porgera,Paiela,Tipinini,Kairik,Koroba,Kopiago,Mendi,Tari,Wabag,Laiagam,Kandep,Kompiam,Wapenamanda,Pogera"
 },
 {
 "name": "Isabi",
 "i": 2030,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Isabi,Goroka,Kainantu,Madang,Alexishafen,Bagabag,Karkar,Mugil,Sumgilbar,Bunabun,Malala,Hatzfeldthafen,Bogia"
 },
 {
 "name": "Italo-Paulista",
 "i": 2034,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Sao Paulo,Santos,Campinas,Jundiai,Sorocaba,Ribeirao Preto,Sao Jose dos Campos,Santo Andre,Osasco,Sao Bernardo do Campo,Piracicaba,Mogi das Cruzes,Sao Jose do Rio Preto,Bauru"
 },
 {
 "name": "Itza",
 "i": 2036,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Tayasal,Flores,San Jose,Peten,San Andres,Santa Elena,San Benito,La Libertad,Sayaxche,Melchor de Mencos,Poptun,Dolores,San Luis,San Francisco"
 },
 {
 "name": "Jimi",
 "i": 2039,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Jimi,Sepik,Waghi,Mount Hagen,Tabibuga,Kol,Koinambe,Togban,Karakai,Bubulsnga,Tsinkol,Wandeke,Ambun,Wurup"
 },
 {
 "name": "Ambakich",
 "i": 2059,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ambunti,Pagwi,Maprik,Wewak,Angoram,Timbunke,Korogo,Kanduanum,Kaminimbit,Mindimbit,Chambri,Aibom"
 },
 {
 "name": "Ap Ma",
 "i": 2060,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Angoram,Timbunke,Keram,Kambot,Biwat,Marienberg,Murik,Karau,Mendam,Kaup,Bien,Watam"
 },
 {
 "name": "Asabano",
 "i": 2061,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Oksapmin,Telefomin,Eliptaman,Tekin,Bimin,Bak,Sisimin,Duranmin,Frieda,May River,Green River,Amanab"
 },
 {
 "name": "Bogaya",
 "i": 2062,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tari,Koroba,Kopiago,Margarima,Komo,Benaria,Pori,Pureni,Tani,Fugwa,Levani,Hewa"
 },
 {
 "name": "Burmeso",
 "i": 2063,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Burmeso,Kustera,Danau Bira,Mamberamo,Sarmi,Bonggo,Demta,Genyem,Lereh,Arso,Waris,Senggi"
 },
 {
 "name": "Mpur",
 "i": 2064,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kebar,Saukorem,Amberbaken,Senopi,Mubrani,Sidey,Pami,Manokwari,Anggi,Ransiki,Oransbari,Windesi"
 },
 {
 "name": "Maibrat",
 "i": 2065,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Ayamaru,Aitinyo,Aifat,Mare,Teminabuan,Sorong,Klamono,Sausapor,Makbon,Beraur,Konda,Inanwatan"
 },
 {
 "name": "Elseng",
 "i": 2067,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Elseng,Dabra,Kustera,Lereh,Genyem,Demta,Sarmi,Bonggo,Arso,Senggi,Waris,Keerom"
 },
 {
 "name": "Kapauri",
 "i": 2068,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kapauri,Pagai,Idano,Kaureh,Lereh,Genyem,Arso,Senggi,Waris,Keerom,Dabra,Kustera"
 },
 {
 "name": "Northern Formosan",
 "i": 2069,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Wulai,Atayal,Seediq,Taroko,Jianshi,Nan'ao,Datong,Fuxing,Jinfeng,Zhuoxi,Wanrong,Xiulin"
 },
 {
 "name": "Philippine",
 "i": 2072,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Manila,Quezon City,Cebu City,Davao City,Zamboanga City,Antipolo,Pasig,Taguig,Valenzuela,Dasmarinas,Bacoor,General Santos"
 },
 {
 "name": "Tsouic",
 "i": 2074,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Alishan,Dabang,Tfuya,Saviki,Tapangu,Sinvi,Lalauya,Namasia,Taoyuan,Maolin,Wanshan,Tona"
 },
 {
 "name": "Greater Central Philippine",
 "i": 2075,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cebu,Davao,Cagayan de Oro,Iloilo,Bacolod,Butuan,Cotabato,General Santos,Ormoc,Tacloban,Tagbilaran,Surigao"
 },
 {
 "name": "Batanic",
 "i": 2076,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Basco,Itbayat,Ivana,Mahatao,Uyugan,Sabtang,Batan,Diogo,Siayan,Mabudis,Ibuhos,Dequey"
 },
 {
 "name": "Setlokwa",
 "i": 2094,
 "min": 4,
 "max": 12,
 "d": "nic-GH",
 "m": 0,
 "b": "Gaborone,Francistown,Molepolole,Selebi-Phikwe,Maun,Kanye,Serowe,Mochudi,Lobatse,Palapye,Jwaneng,Mahalapye"
 },
 {
 "name": "Kamono",
 "i": 2098,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kainantu,Arona,Kassam,Yonki,Obura,Barola,Raipinka,Norikori,Punano,Aiyura"
 },
 {
 "name": "Kanasi",
 "i": 2100,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Alotau,Samarai,Esa'ala,Losuia,Misima,Dogura,Baniara,Rabaraba,Wedau,Sideia"
 },
 {
 "name": "Kandawo",
 "i": 2101,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Mount Hagen,Minj,Banz,Kudjip,Tambul,Nebilyer,Baiyer,Jimi,Kagamuga,Togoba"
 },
 {
 "name": "Kanite",
 "i": 2103,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Okapa,Lufa,Henganofi,Tarabo,Keyagana,Gimi,Yagaria,Fore,Bena,Kamano"
 },
 {
 "name": "Kap",
 "i": 2106,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Madang,Alexishafen,Bagabag,Karkar,Mugil,Sumgilbar,Bunabun,Malala,Hatzfeldthafen,Bogia"
 },
 {
 "name": "Kashinawa",
 "i": 2112,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "kashinawa,Purus,Tarauaca,Jordao,Feijo,Santa-Rosa-do-Purus,Manoel-Urbano,Sena-Madureira,Rio-Branco,Cruzeiro-do-Sul,Envira,Jurua"
 },
 {
 "name": "Camsa",
 "i": 2115,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sibundoy,Colon,San-Francisco,Santiago,Mocoa,Villagarzon,Puerto-Guzman,Puerto-Asis,Puerto-Caicedo,Orito,Valle-del-Guamuez,San-Miguel"
 },
 {
 "name": "North Vanuatu",
 "i": 2119,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Torres,Banks,Santo,Malo,Ambae,Maewo,Pentecost,Malakula,Ambrym,Paama,Epi,Shepherd"
 },
 {
 "name": "St. Matthias",
 "i": 2123,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Mussau,Emira,Tench,Eloaua,Boliu,Lomakalau,Malakata,Palakau,Taskul,Etalau,Enusat,Erapun"
 },
 {
 "name": "Temotu",
 "i": 2124,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Santa-Cruz,Utupua,Vanikoro,Tikopia,Anuta,Fatutaka,Duff,Reef,Lata,Nendo,Graciosa,Noole"
 },
 {
 "name": "Vanuatu",
 "i": 2125,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Port-Vila,Luganville,Norsup,Isangel,Lakatoro,Sola,Saratamata,Lenakel,Pango,Mele,Ifira,Erakor"
 },
 {
 "name": "Keyagana",
 "i": 2134,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goroka,Kainantu,Henganofi,Lufa,Okapa,Daulo,Obura-Wonenara,Unggai-Bena,Aiyura,Arona,Yonki,Kassam"
 },
 {
 "name": "Kewa",
 "i": 2140,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Mendi,Ialibu,Pangia,Kagua,Erave,Nipa,Tari,Koroba,Margarima,Komo,Lake-Kutubu,Kandep"
 },
 {
 "name": "Admiralty",
 "i": 2149,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lorengau,Manus,Los-Negros,Pak,Rambutyo,Lou,Baluan,MBuke,Bipi,Hermit,Ninigo,Wuvulu"
 },
 {
 "name": "Afroasiatic family",
 "i": 2150,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Cairo,Riyadh,Addis-Ababa,Algiers,Casablanca,Khartoum,Mogadishu,Tunis,Tripoli,Beirut,Amman,Sanaa"
 },
 {
 "name": "Alor-Pantar",
 "i": 2151,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kalabahi,Baranusa,Kabir,Blangmerang,Maritaing,Moru,Apui,Alor-Kecil,Kokar,Mainang,Wolwal,Matar"
 },
 {
 "name": "Angas family",
 "i": 2152,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Pankshin,Kabwir,Dawaki,Amper,Chip,Tal,Pai,Fier,Tambes,Garram,Shiwer,Kanam"
 },
 {
 "name": "Anim languages",
 "i": 2153,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Merauke,Okaba,Kimaam,Muting,Jeti,Kurik,Tanah-Miring,Semangga,Sota,Ulilin,Eligobel,Tubang"
 },
 {
 "name": "Aru",
 "i": 2154,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dobo,Benjina,Marlasi,Koijabi,Wakua,Lola,Taberfane,Rebi,Jila,Warialau,Kofiau,Misool"
 },
 {
 "name": "Awin-Pa",
 "i": 2156,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kiunga,Tabubil,Ningerum,Olsobip,Rumginae,Drimdenasuk,Gre,Atkamba,Kungim,Haewenai,Bige,Menemsore"
 },
 {
 "name": "Awyu-Dumut",
 "i": 2157,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tanah-Merah,Bade,Kouh,Bomakia,Senggo,Citak,Wildeman,Digul,Mappi,Edera,Obaa,Passue"
 },
 {
 "name": "Bade family",
 "i": 2158,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Gashua,Jakusko,Badewa,Amshi,Tagali,Katamma,Dagona,Gorgoram,Usur,Sugum,Jajimaji,Karage"
 },
 {
 "name": "Kimaama-Kimaghama",
 "i": 2168,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kimaam,Bamol,Kalilam,Konkondau,Wanam,Idoor,Teri,Yud,Kiki,Sabon,Wan,Yur"
 },
 {
 "name": "Central Vanuatu",
 "i": 2172,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Port Vila,Efate,Epi,Tongoa,Shepherd,Nguna,Pele,Emae,Makira,Mataso,Tongariki,Buninga"
 },
 {
 "name": "Eastern Oceanic",
 "i": 2173,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Honiara,Port Vila,Suva,Nuku'alofa,Apia,Noumea,Mata-Utu,Alofi,Avarua,Adamstown,Papeete,Majuro"
 },
 {
 "name": "Meso-Melanesian",
 "i": 2174,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Rabaul,Kavieng,Buka,Namatanai,Kokopo,Kimbe,Lorengau,Hoskins,Bialla,Kandrian,Gasmata,Pomio"
 },
 {
 "name": "Oceanic",
 "i": 2175,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Funafuti,Tarawa,Yaren,Palikir,Majuro,Port Vila,Honiara,Suva,Nuku'alofa,Apia,Noumea,Mata-Utu"
 },
 {
 "name": "Binanderean",
 "i": 2176,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Popondetta,Kokoda,Ioma,Mambare,Gona,Buna,Oro Bay,Higaturu,Safia,Afore,Tufi,Wanigela"
 },
 {
 "name": "Kutubuan languages",
 "i": 2177,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lake Kutubu,Moro,Pimaga,Baguale,Hegigio,Mubi,Digimu,Kantobo,Foi,Fasu,Namumi,Some"
 },
 {
 "name": "Goilalan",
 "i": 2178,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tapini,Woitape,Guari,Loloipa,Kataipa,Kunimaipa,Tauade,Fuyug,Ononge,Yongai,Kerau,Kamulai"
 },
 {
 "name": "Kolopom",
 "i": 2179,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kimaam,Frederik Hendrik,Dolak,Pulau Yos Sudarso,Okaba,Ilwayab,Tabonji,Waan,Kimam,Kalilam,Bamol,Kladar"
 },
 {
 "name": "Damal",
 "i": 2180,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Beoga,Ilaga,Sinak,Agadugume,Jila,Bela,Alama,Wangbe,Dagi,Kembru,Pogapa,Hitadipa"
 },
 {
 "name": "Dem",
 "i": 2181,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dem,Puncak-Jaya,Mulia,Sinak,Beoga,Ilaga,Yamo,Mewoluk,Tingginambut,Fawi,Jila,Bela"
 },
 {
 "name": "Dibiyaso",
 "i": 2182,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Bosavi,Lake-Campbell,Wawoi,Turama,Bamu,Mount-Bosavi,Fogomaiyu,Libano,Aiba,Walagu,Musula,Bona"
 },
 {
 "name": "Guriaso",
 "i": 2183,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Amanab,Green-River,Imonda,Waris,Yapsiei,Edwaki,Pagei,Sowanda,Kamberatoro,Utai,Bewani,Lumi"
 },
 {
 "name": "Kaki Ae",
 "i": 2184,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kerema,Ihu,Baimuru,Malalaua,Murua,Moveave,Terapo,Silo,Hamuhamu,Mei,Opau,Karama"
 },
 {
 "name": "Karami",
 "i": 2185,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Daru,Morehead,Kiunga,Tabubil,Nomad,Balimo,Weam,Bensbach,Suki,Lake-Murray,Rouku,Buji"
 },
 {
 "name": "Kehu",
 "i": 2186,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kehu,West-Papua,Nabire,Enarotali,Moanemani,Mapia,Siriwo,Uwapa,Wanggar,Yaur,Teluk-Etna,Kaimana"
 },
 {
 "name": "Kaiwa",
 "i": 2187,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Dourados,Amambai,Ponta-Pora,Caarapo,Iguatemi,Itapora,Maracaju,Rio-Brilhante,Navirai,Mundo-Novo,Eldorado,Japora"
 },
 {
 "name": "Kiwai",
 "i": 2238,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Daru,Kiway,Wabuda,Bamu,Turama,Kikori,Goaribari,Sui,Mabudawan,Parama,Tureture,Katatai"
 },
 {
 "name": "Klon",
 "i": 2241,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Probur,Habolat,Halerman,Wolwal,Bukapiting,Alila,Aimoli,Pintumas,Moru,Manet,Mauta,Kui"
 },
 {
 "name": "Akatek",
 "i": 2249,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "San Miguel Acatan,San Rafael La Independencia,Nenton,Jacaltenango,Concepcion Huista,Petatan,San Antonio Huista,Santa Ana Huista,Todos Santos Cuchumatan,Huehuetenango,Chiantla,Aguacatan"
 },
 {
 "name": "Koitabu",
 "i": 2253,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Port Moresby,Hanuabada,Kila Kila,Baruni,Tatana,Pari,Vabukori,Kilakila,Korobosea,Gerehu,Waigani,Boroko"
 },
 {
 "name": "Komolom",
 "i": 2256,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Yos Sudarso,Kimaam,Kimaghama,Riantana,Merauke,Okaba,Kaptel,Muting,Jibi,Waan,Tabonji,Kolam"
 },
 {
 "name": "Komyandaret",
 "i": 2257,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sarmi,Mamberamo,Bonggo,Lereh,Genyem,Nimboran,Kemtuik,Gresi,Kwansu,Orya,Bagusa,Kwerba"
 },
 {
 "name": "Konai",
 "i": 2258,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kiunga,Tabubil,Olsobip,Ningerum,Rumginae,Drimdenasuk,Gre,Dahamo,Atkamba,Kungim,Membok,Biaka"
 },
 {
 "name": "Koneraw",
 "i": 2261,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Merauke,Okaba,Kaptel,Muting,Jibi,Waan,Tabonji,Kolam,Kimaam,Kimaghama,Riantana,Yos Sudarso"
 },
 {
 "name": "Kopkaka",
 "i": 2264,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tanah Merah,Kouh,Bomakia,Mandobo,Yaniruma,Wanggom,Kombai,Korowai,Citak,Mitak,Auyu,Asmat"
 },
 {
 "name": "Korafe",
 "i": 2266,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tufi,Oro Bay,Popondetta,Kokoda,Afore,Ioma,Kira,Safia,Sangara,Sairope,Gona,Buna"
 },
 {
 "name": "Kosena",
 "i": 2272,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Okapa,Kainantu,Lufa,Henganofi,Obura-Wonenara,Goroka,Unggai-Bena,Daulo,Yonkki,Arona,Purosa,Tarabo"
 },
 {
 "name": "Kovojab",
 "i": 2273,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Madang,Karkar,Bagabag,Alexishafen,Amele,Bel,Gum,Isumrud,Mugil,Rempi,Sek,Yabob"
 },
 {
 "name": "Karaja",
 "i": 2275,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Santa Isabel do Morro,Fontoura,Sao Domingos,Luciara,Sao Felix do Araguaia,Bananal,Javae,Karaja,Xambioa,Aruana,Crixas,Araguacu"
 },
 {
 "name": "Kubo",
 "i": 2284,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Nomad,Mougulu,Yehebi,Honinabi,Oksapmin,Telefomin,Kiunga,Tabubil,Ningerum,Olsobip,Gobe,Kantobo"
 },
 {
 "name": "Kui Alor-Pantar",
 "i": 2285,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kalabahi,Moru,Baranusa,Kabir,Lantoka,Mebung,Alor,Pantar,Pura,Ternate,Kepola,Kailesa"
 },
 {
 "name": "Kula",
 "i": 2289,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Kalabahi,Moru,Lantoka,Mebung,Baranusa,Kabir,Pura,Ternate,Pantar,Alor,Abang,Aitui"
 },
 {
 "name": "Kyaka",
 "i": 2303,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Goroka,Kainantu,Kundiawa,Mendi,Mt Hagen,Wabag,Chimbu,Jiwaka,Hela,Enga"
 },
 {
 "name": "Lingua Geral Paulista",
 "i": 2309,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sao Paulo,Campinas,Sao Bernardo do Campo,Santo Andre,Sao Jose dos Campos,Sorocaba,Santos,Osasco,Ribeirao Preto,Sao Jose do Rio Preto"
 },
 {
 "name": "Ber Family",
 "i": 2330,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Agadir,Casablanca,Fes,Marrakech,Tanger,Meknes,Oujda,Kenitra,Rabat,Sale"
 },
 {
 "name": "Azd Dialect",
 "i": 2345,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "-Muscat,Salalah,Sohar,Seeb,Bawshar,Sur,Ibra,Rustaq"
 },
 {
 "name": "Ejtun Dialect",
 "i": 2346,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "-Mosta,Victoria,Nadur,Qormi,Åebbua¡,Xaga§ra,Safi,Kirkop"
 },
 {
 "name": "L Ngua Geral Paulista",
 "i": 2394,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Sao Paulo,Campinas,Sao Bernardo do Campo,Santo Andre,Sao Jose dos Campos,Sorocaba,Santos,Osasco,Ribeirao Preto,Sao Jose do Rio Preto"
 },
 {
 "name": "Laua",
 "i": 2407,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Laua,Mendi,Tari,Koroba,Nipa,Poroma,Ialibu,Pangia,Wabag,Kandep,Lagaip,Porgera,Kompiam,Wapenamanda,Baiyer,Mul,Minj,Kudjip,Hagen,Tambul,Dei,Komo,Magarima,Hulia,Koroba Station,Beneria,Pureni,Fugwa,Hewa,Duna"
 },
 {
 "name": "Lauje",
 "i": 2410,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Tinombo,Moutong,Parigi,Tomini,Ampibabo,Toribulu,Balinggi,Sidoan,Ogotua,Kasimbar,Palasa,Dampelas,Balaesang,Damsol,Sausu"
 },
 {
 "name": "Lembena",
 "i": 2415,
 "min": 4,
 "max": 11,
 "d": "nic-GH",
 "m": 0,
 "b": "Lembena,Mendi,Nipa,Tari,Koroba,Poroma,Ialibu,Pangia,Kagua,Erave,Margarima,Hulia,Beneria,Pureni,Fugwa,Hewa,Duna,Kopiago,Pori,Komo,Lai,Nembi,Wiru,Mbo,Imbonggu,Wola,Kewa,Fasu,Kubo,Foi"
 },
 {
 "name": "Angami-Pochuri",
 "i": 2478,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Kohima,Viswema,Kigwema,Jakhama,Zakhama,Khonoma,Jotsoma,Tuophema,Medziphema,Phek,Chozuba,Chetheba,Pfutsero,Meluri,Chizami,Thenyizu,Sakraba,Khezhakeno,Porba,Pholami"
 },
 {
 "name": "Akan",
 "i": 2507,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Kumasi,Accra,Koforidua,Obuasi,Tema,Konongo,Ejisu,New-Tafo,Offinso,Sunyani,Techiman,Bechem,Mampong,Asamankese,Nkawkaw,Agogo,Juaso,Akosombo,Winneba,Cape-Coast"
 },
 {
 "name": "Qeqchi",
 "i": 2548,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Coban,Raxruha,Chisec,Fray-Bartolome,San-Pedro-Carcha,San-Cristobal-Verapaz,Tactic,Tamahu,Panzos,Senahu,Lanquin,Chahal,El-Estor,La-Tinta,Santa-Cruz-Verapaz,Frayba"
 },
 {
 "name": "Kiche",
 "i": 2549,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Quetzaltenango,Totonicapan,Momostenango,Santa-Cruz-del-Quiche,Joyabaj,Chichicastenango,Nahuala,San-Andres-Xecul,Salcaja,San-Cristobal-Totonicapan,San-Francisco-El-Alto,Chicaman,Pachalum,Patzite,Canilla,Zunil,Sibilia"
 },
 {
 "name": "Tzeltal",
 "i": 2552,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Ocosingo,Altamirano,Chanal,Oxchuc,Amatenango,Teopisca,Tenejapa,San-Cristobal,Abasolo,Agua-Azul,Sitala,Chilon,Bachajon,Nuevo-Jerusalem,Petalcingo,El-Bosque"
 },
 {
 "name": "Tzotzil",
 "i": 2554,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "San-Cristobal-de-las-Casas,Zinacantan,Chamula,Huixtan,Teopisca,Tenejapa,Amatenango-del-Valle,Larrainzar,Chalchihuitan,Chenalho,Oxchuc,Pantelho,Bochil,Soyalo"
 },
 {
 "name": "Totonac",
 "i": 2558,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Papantla,Poza-Rica,Coatzintla,Espinal,Cazones,Zozocolco,Chumatlan,Filomeno-Mata,Martinez-de-La-Torre,Tlapacoyan,Misantla,Naolinco,Teziutlan,Zacapoaxtla"
 },
 {
 "name": "Chol",
 "i": 2559,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Tila,Tumbala,Sabanilla,Salto-de-Agua,Palizada,Palenque,La-Libertad,Yajalon,Chilon,Ocosingo,Tenosique,Balancan,Emiliano-Zapata,Macuspana"
 },
 {
 "name": "Qanjobal",
 "i": 2561,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Santa-Eulalia,San-Juan-Ixcoy,San-Pedro-Solon,Soloma,San-Rafael-La-Independencia,San-Miguel-Acatan,San-Sebastian-Coatan,Barillas,Nenton,Jacaltenango"
 },
 {
 "name": "Huastec",
 "i": 2562,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Ciudad-Valles,Tampico,Mante,El-Naranjo,Tamuin,Tancanhuitz,Axtla,Matlapa,Xilitla,Aquismon,Huejutla,Tempoal,Panuco,Tantoyuca,Chicontepec"
 },
 {
 "name": "Wayuu",
 "i": 2568,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Riohacha,Maicao,Uribia,Paraguachon,Cabo-de-la-Vela,Puerto-Lopez,El-Molinillo,Mingueo,Dibulla,Barrancas,Hatonuevo,San-Juan-del-Cesar,Cuestecitas"
 },
 {
 "name": "Tikuna",
 "i": 2580,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Leticia,Tabatinga,Benjamin Constant,Santo Antonio,Sao Paulo Olivenca,Puerto Narino,Santa Sofia,Nazaret,San Martin,Tarapoto,Atalaia do Norte,Amatura,Tonantins,Islandia,San Pedro,Santa Rita,Caballococha,Yavari,San Pablo,Cushillo Cocha,Tres Fronteras,Lago Tarapoto,Puerto Alegria,Puerto Arica,Santa Rosa,Pucaurquillo,Bellavista,Pevas,Nuevo Israel,San Jose Amazonas"
 },
 {
 "name": "Tlingit",
 "i": 2611,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Sitka,Juneau,Hoonah,Yakutat,Kake,Angoon,Haines,Skagway,Pelican,Tenakee,Klukwan,Wrangell,Ketchikan,Saxman,Metlakatla,Hydaburg,Kasaan,Craig,Thorne Bay,Klawaock"
 },
 {
 "name": "Haida",
 "i": 2612,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Skidegate,Old Massett,Masset,Port Clements,Queen Charlotte,Sandspit,Tlell,Prince Rupert,Lax Kw'alaams,Kitkatla,Metlakatla Village,Hydaburg,Kasaan,Alert Bay"
 },
 {
 "name": "Sranan",
 "i": 2654,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Paramaribo,Nieuw Nickerie,Lelydorp,Moengo,Albina,Nieuw Amsterdam,Brownsweg,Groningen,Wageningen,Totness,Onverwacht,Brokopondo,Tamanredjo,Domburg,Saramacca,Coronie,Commewijne,Marowijne,Brokopondo Centrum,Meerzorg"
 },
 {
 "name": "Arawak",
 "i": 2655,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Georgetown,Bartica,Linden,New Amsterdam,Anna Regina,Lethem,Paramaribo,Albina,Kourou,Cayenne,Saint-Laurent,Maripasoula,Mana,Iracoubo,Sinnamary,Apatou,Moengo,Galibi,St Georges,Ouanary"
 },
 {
 "name": "Innu",
 "i": 2662,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Sheshatshiu,Natuashish,Maliotenam,Uashat,Matimekush,Lac-John,Ekuanitshit,Pessamit,Essipit,Unamen Shipu,La Romaine,Mingan,Schefferville,Sept-Iles,Baie-Comeau,Forestville,Port-Cartier,Chute-aux-Outardes,Longue-Pointe-de-Mingan,Havre-Saint-Pierre"
 },
 {
 "name": "Greenlandic",
 "i": 2668,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Nuuk,Sisimiut,Ilulissat,Qaqortoq,Aasiaat,Maniitsoq,Narsaq,Paamiut,Tasiilaq,Ittoqqortoormiit,Uummannaq,Upernavik,Nanortalik,Qasigiannguit,Qeqertarsuaq,Kangerlussuaq,Qeqertarsuatsiaat,Kullorsuaq,Savissivik,Illoqqortoormiut"
 },
 {
 "name": "Aleut",
 "i": 2681,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Unalaska,Akutan,Atka,Adak,Nikolski,King Cove,False Pass,Sand Point,Cold Bay,Dutch Harbor,Biorka,Chernofski,Kaguyak,Kashega,Kinak,Makushin,Morzhovoi,Nazan,Pauloff,Popof,Qagan Toyagungin,Reef Bay,Sedanka,Squaw Harbor,Tanax Angunax,Tigalda,Unga,Uyak,Voznesenka"
 },
 {
 "name": "Yupik",
 "i": 2682,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Bethel,Dillingham,Ekwok,Emmonak,Hooper Bay,Kotlik,Kwethluk,Napaskiak,Nunapitchuk,Oscarville,Pilot Station,Platinum,Quinhagak,Red Devil,Sleetmute,Stebbins,Togiak,Toksook Bay,Tuluksak,Tuntutuliak,Tununak,Nightmute,Chefornak,Chevak,Kongiganak,Kwigillingok,Mekoryuk,Newtok,Akiachak,Akiak"
 },
 {
 "name": "Athabaskan",
 "i": 2683,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Fairbanks,Anchorage,Juneau,Sitka,Ketchikan,Wrangell,Petersburg,Cordova,Valdez,Seward,Homer,Kenai,Soldotna,Wasilla,Palmer,Eagle River,Chugiak,Kodiak,Bethel,Nome,Utqiagvik,Kotzebue,Prudhoe Bay,Deadhorse,Fort Yukon,Tanana,Galena,Unalakleet"
 },
 {
 "name": "Tokelauan",
 "i": 2684,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Atafu,Fakaofo,Nukunonu,Fale,Tokelau,Atafu Village,Fakaofo Village,Nukunonu Village,Fenuafala,Fale islet"
 },
 {
 "name": "Nar-Phu",
 "i": 2686,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Manang,Nar,Phu,Chame,Pisang,Braga,Ngawal,Ghyaru,Humde,Khangsar,Tilicho,Thorong,Muktinath,Kagbeni,Jomsom,Marpha,Tukuche,Larjung,Lete,Ghasa,Dana,Tatopani,Beni,Baglung,Pokhara,Gorkha,Lamjung,Syangja,Parbat,Myagdi"
 },
 {
 "name": "Awa",
 "i": 2734,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Tulcan,Ipiales,San Gabriel,Mira,Espejo,Montufar,Huaca,Julio Andrade,Cristobal Colon,Chitan,Tufiño,Maldonado,Chical,Tobar Donoso,Lita,Alto Tambo,Ricaurte,Tumaco,Barbacoas,Magui,Payan"
 },
 {
 "name": "Awadhi",
 "i": 2735,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Lucknow,Kanpur,Allahabad,Varanasi,Faizabad,Ayodhya,Sultanpur,Pratapgarh,Rae Bareli,Unnao,Hardoi,Sitapur,Lakhimpur,Bahraich,Gonda,Basti,Gorakhpur,Azamgarh,Jaunpur,Mirzapur,Banda,Fatehpur,Bareilly,Shahjahanpur,Pilibhit"
 },
 {
 "name": "Awbono",
 "i": 2736,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Oksibil,Tanah Merah,Boven Digoel,Merauke,Mindiptana,Waropko,Jair,Kouh,Ninati,Kawagit,Senggo,Bupul,Kepi,Muting,Getentiri,Tanahmerah,Kimaam,Okaba,Kurik,Elikobel,Sota,Naukenjerai"
 },
 {
 "name": "Awin",
 "i": 2737,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Kiunga,Ningerum,Olsobip,Tabubil,Rumginae,Ambunti,Pagwi,Maprik,Wewak,Angoram,Timbunke,Wagu,Hunstein,Amboin,Oum,Sio,Green River,Vanimo,Aitape,Lumi,Nuku,Dreikikir,Yangoru,Kubalia"
 },
 {
 "name": "Be-Jizhao",
 "i": 2739,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Nanning,Beihai,Qinzhou,Fangchenggang,Dongxing,Pingxiang,Chongzuo,Ningming,Longzhou,Daxin,Tiandeng,Fusui,Shangsi,Pubei,Lingshan,Hepu,Bobai,Yulin,Beiliu,Xingye,Rongxian,Luchuan,Guiping,Pingnan"
 },
 {
 "name": "Be",
 "i": 2740,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Lingao,Haikou,Danzhou,Dongfang,Changjiang,Baisha,Qiongzhong,Tunchang,Chengmai,Lingshui,Wanning,Qionghai,Wenchang,Dingan,Ledong,Sanya,Baoting,Wuzhishan,Sansha,Yangpu,Meilan,Longhua"
 },
 {
 "name": "Beami",
 "i": 2741,
 "min": 4,
 "max": 11,
 "d": "lnrt",
 "m": 0,
 "b": "Bosavi,Kaluli,Ologo,Walulu,Wisesi,Kikori,Strickland,Nomad,Komofio,Mougulu,Sulamesi,Bedamini"
 },
 {
 "name": "Rarotongan",
 "i": 2750,
 "min": 4,
 "max": 12,
 "d": "lnrt",
 "m": 0,
 "b": "Avarua,Takuvaine,Matavera,Ngatangiia,Nikao,Tupapa,Aitutaki,Atiu,Mitiaro,Mauke,RarotongaTown,AvaruaTown,TakuvaineTown,MataveraTown"
 }
];
