...
// Experimental blended compound Markov helper for dev use.
//
// Goal: generate names where a SINGLE name can contain segments from
// MULTIPLE bases (e.g. Kx'a + Germanic in one string), using the same
// Markov/length heuristics as the app, plus boundary smoothing and
// probabilistic spaces/hyphens for romanized segments.
//
// This script is standalone and does NOT affect the in-browser generator.
// Use it only as a playground when tuning languages.
//
// Usage examples (from project root):
//   node tools/experiment-compound-markov-blend.js --base=353,354 --count=40 --min=15 --max=50 --seed=1
//   node tools/experiment-compound-markov-blend.js --base=0,1,6 --count=40 --min=15 --max=50 --seed=1
...
// Copied from utils/languageUtils.js (see previous session context)
const VOWELS = "aeiouy'";
function vowel(c) {
...
function generateBlendedName(contexts, rng, opts) {
  const requestedMin = typeof opts.min === "number"
    ? opts.min
    : Math.min(...contexts.map(c => c.base.min || 4));
  const requestedMax = typeof opts.max === "number"
    ? opts.max
    : Math.max(...contexts.map(c => c.base.max || (requestedMin + 4)));
  ...
  while (total < requestedMin && guard < maxSegments) {
    ...
    const segMin = Math.max(2, Math.min(Math.round(segMean), base.max || Math.round(segMean) + 2));
    const segMax = Math.max(segMin + 1, Math.min(base.max || segMin + 4, Math.round(segMean + 2)));

    const seg = generatePlainName(base, ctx.chain, rng, {
      min: segMin,
      max: segMax,
      dupl: base.d || "",
    });

    segments.push({text: seg, ctx});
    total += seg.length;
    guard++;
  }
  ...
  let compound = segments[0].text;
  for (let i = 1; i < segments.length; i++) {
    const seg = segments[i];
    compound = smoothJoin(compound, seg.text, seg.ctx.onsetSet, rng);
  }

  // If still too short and we haven't hit maxSegments, try to append one more.
  let extraGuard = 0;
  while (compound.length < requestedMin && segments.length < maxSegments && extraGuard < 2) {
    const ctx = contexts[Math.floor(rng() * contexts.length)];
    const stats = ctx.stats;
    const base = ctx.base;
    ...
    const segName = generatePlainName(base, ctx.chain, rng, {
      min: segMin,
      max: segMax,
      dupl: base.d || "",
    });

    compound = smoothJoin(compound, segName, ctx.onsetSet, rng);
    segments.push({text: segName, ctx});
    extraGuard++;
  }

  if (requestedMax != null && compound.length > requestedMax + 10) {
    const ctx = contexts[Math.floor(rng() * contexts.length)];
    const base = ctx.base;
    const fallback = generatePlainName(base, ctx.chain, rng, {
      min: requestedMin,
      max: requestedMax,
      dupl: base.d || "",
    });
    return {text: fallback, bases: [ctx.idx]};
  }

  const usedIdxs = Array.from(new Set(segments.map(s => s.ctx.idx))).sort((a, b) => a - b);
  return {text: compound, bases: usedIdxs};
}
...
function parseArgs(argv) {
  ...
  const modeArg = getValue("--mode"); // accepted but not used; kept for CLI symmetry
  ...
  return {baseIndices, count, min, max, seed, mode, help};
}

function printUsage() {
  console.log(
    "Usage: node tools/experiment-compound-markov-blend.js --base=IDX[,IDX...] [options]\n",
  );
  console.log("Options:");
  console.log("  --base=IDX[,IDX...]   One or more base indices (e.g. 353,354). Required.");
  console.log("  --count=N            Number of blended names to generate (default 40).");
  console.log("  --min=INT            Target minimum total length.");
  console.log("  --max=INT            Target maximum total length.");
  console.log("  --mode=plain|auto    Accepted for symmetry; currently always blended (default auto).");
...
function main() {
  const {baseIndices, count, min, max, seed, mode, help} = parseArgs(process.argv);
  ...
  console.log("=== Experimental blended compound Markov ===");
  ...
  const lengths = [];
  console.log("-- Blended output --");
  for (let i = 0; i < count; i++) {
    const {text, bases: usedIdxs} = generateBlendedName(contexts, rng, {min, max});
  ...
  if (lengths.length) {
    ...
    console.log("");
    console.log(
      `  [lengths] count=${c} min=${minL} max=${maxL} mean=${mean.toFixed(2)} p25=${q(
        0.25,
...
if (require.main === module) {
  try {
    ...
    console.error(
      "Error in experiment-compound-markov-blend:",
