...
const cityReplacements = {
  ...
  'laiuse-romani': 'Tartu,Võru,Põlva,Jõgeva,Valga,Viljandi,Pärnu,Rakvere,Kuressaare,Narva',
...
for (const [languagePattern, cityNames] of Object.entries(cityReplacements)) {
  // Match pattern: language_pattern_XXXXX_unq1,language_pattern_XXXXX_unq2,...
  // Also match: language_pattern_unq1,language_pattern_unq2,...
  const regex1 = new RegExp(`${languagePattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\d+_unq\\d+`, 'g');
  const regex2 = new RegExp(`${languagePattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}_unq\\d+`, 'g');

  const count1 = (content.match(regex1) || []).length;
  const count2 = (content.match(regex2) || []).length;

  if (count1 > 0) {
    content = content.replace(regex1, cityNames);
    replacementsMade += count1;
    replacementCounts[languagePattern] = (replacementCounts[languagePattern] || 0) + count1;
    console.log(`Replaced ${count1} occurrences of ${languagePattern}\\d+_unq*`);
  }

  if (count2 > 0) {
    content = content.replace(regex2, cityNames);
    replacementsMade += count2;
    replacementCounts[languagePattern] = (replacementCounts[languagePattern] || 0) + count2;
    console.log(`Replaced ${count2} occurrences of ${languagePattern}_unq*`);
  }
}
...
const remainingUnq = content.match(/[^a-z]unq\d+/gi);
console.log(`\nRemaining unq patterns: ${remainingUnq ? remainingUnq.length : 0}`);
...
// Summary by language
console.log('\nReplacement summary:');
for (const [lang, count] of Object.entries(replacementCounts).sort((a, b) => b[1] - a[1])) {
